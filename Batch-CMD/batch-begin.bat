...
echo В переменной VARSTR хранится значение %VARSTR% & REM Для подстановки значения переменной её нужно обрамлять с двух сторон символами % %
...
echo %ARGDEMO%& REM Особенность: Переменные, присвоенные внутри процедуры (внешнего скрипта) доступны и после её выполнения. А значит могут быть потеряны значения, которые в них были до вызова процедуры.
    ...
    REM Функция - это блок программы, который прнимает на вход аргументы и возвращает некое значение (или значения). В BAT-скриптах функции не предусмотрены. Но всё-таки  (помимо изменения глобальных переменных) значения можно "возвращать" либо печатью текста в стандартное устройство вывода, либо через передачу 32-битного числового значения ошибки. В первом случае требуется запуск функции из внешнего файла. Для удобства несколько функций объединены в одном внешнем скрипте batch-lib.bat
...
for /f "delims=" %%A in ('batch-lib.bat func1 %STRING1%') do set STRING2=%%A& REM Функция принимает на вход STRING1 и возвращает STRING2
...
call batch-lib.bat func1 %STRING1% STRING3 > nul
