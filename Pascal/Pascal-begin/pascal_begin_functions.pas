...
interface { В разделе interface объявляются публичные переменные и публичные подпрограммы модуля,
  ...
  function factorial2(n:word):Qword;  // Вычисление факториала n! через множественный выбор
  function prime(n:word):boolean;     // Определение, является число простым
  procedure countdown(n:qword);       // Процедура с хвостовой рекурсией
...
implementation { В разделе implementation описываются тела подпрограмм.
  ...
  procedure countdown(n:qword);
  begin
     if n = 0 then begin writeln; exit; end;
     if n mod 1000000 = 0 then write(n, ' ');
     countdown(n-1);
  end;
