...
interface { В разделе interface объявляются публичные переменные и публичные подпрограммы модуля,
  ...
  uses SysUtils, Math;  // Подключение других модулей делается в начале раздела interface

  var str1: string = 'Глобальное значение';

  procedure proc1; // Декларация простейшей процедуры
  // Этот комментарий, написанный перед объявлением процедуры, будет отображаться в подсказке IDE
  procedure proc2;
  procedure proc3;
  ...
  var func_pointer1: FuncType1; //Переменная, которая может хранить ссылку на функцию типа FuncType1
  ...
  function ReturnFunc1(n:integer):FuncType1;               // Функция, возвращающая функцию.
  function sum(a,b:integer):integer;                       // Функция суммирования двух чисел
  procedure localProc1;
  procedure lexEnvTest;
  function decoString(str:string):string; // Ф-я обработки строки по вшитому в подфункции шаблону
  function sum(a,b:string):string; // Полиморфная функция "суммирования" (соединения) двух строк

  // Функции для демонстрации рекурсии
    // Функция вычисления факториала n!
    function factorial(n:word):Qword;
    // Функция вычислени суммы натуральных чисел от 1 до n
    function summNatural(n:word):Qword;
    // Функция вычисления факториала n! через множественный выбор
    function factorial2(n:word):Qword;
    // Функция определяет, является ли число простым
    function prime(n:word):boolean;
    // Рекурсивная функция определения суммы округлённых квадратных корней чисел от 1 до N:
    function sumSqrt(n:qword):qword;
    // Модифицированная функция с аккумулятором для эмуляции хвостовой рекурсии.
    function sumSqrtTail(var n,acc:qword):boolean;
    // Тип для создания переменных, ссылающихся на функции вида sumSqrtTail
    type TailRecursionType = function (var a,b:qword):boolean;

    { Универсальная функция-трамплин, выполняющая в цикле переданную ей псевдорекурсивную функцию.
      Использование цикла вместо рекурсии позволяет избежать переполнения стека.
      При этом сохраняется возможность через псевдорекурсивные функции задавать рекурсивную логику.}
    function trampoline(f:TailRecursionType; n:qword):qword;
...
implementation { В разделе implementation описываются тела подпрограмм.
  ...
  procedure proc2;
  var s1, s2, str1: string; // Объявление переменных внутри подпрограммы
  begin
    str1 := 'Локальное значение';
    { При совпадении имён локальной и глобальной переменной в теле подпрограммы  используется
      ...
      тела подпрограммы. }
    writeln('Значение локальной переменной str1: ', str1);
    // Доступ к одноимённой глобальной переменной - через добавление имени модуля или программы:
    writeln('Значение глобальной переменной str1: ', Pascal_Begin_Functions.str1);
    // Глобальную переменную можно менять:
    Pascal_Begin_Functions.str1 := 'Новое глобальное значение';
    writeln('Новое значение глобальной переменной str1: ', Pascal_Begin_Functions.str1);
  end;
  ...
  function ReturnFunc1(n:integer):FuncType1; // Функция возвращает разные функции
  begin
    if odd(n) then result:=@summ_sqr         // Для чётных чисел возвращается функция summ_sqr()
      else result:=@sum;                     // для нечётных - sum()
  end;
  ...
  // Видимость внешних переменных у вложенных подпрограмм разных видов
  procedure lexEnvTest;
    var str1: string;
    function func1:string;
      begin result:=str1; end;
  begin
    str1 := 'Локальная переменная';
    writeln('Вложенная функция func1() видит, что str1 - ', func1); // локальная
  end;

  // Простое использование вложенных функций
  function decoString(str:string):string; // Ф-я обработки строки по вшитому в подфункции шаблону
    function deco(c:string; n:integer):string; // Ф-я декорирования строки по краям другой строкой
      function mul(reverse:boolean):string;      // Функция повторения декорируемой строки
      var i,j:integer; s,t:string;
      begin s:='';
        for i:=1 to n do s+=c;
        t:=s;
        if reverse then for j:=1 to length(s) do s[j]:=t[length(s)-j+1]; // Разворот строки
        result := s;
      end;
    begin
      result := mul(false) + str + mul(true);
    end;
  begin
    result := deco('*-',3);
  end;

  // Можно объявлять полиморфные функции - с одинаковыми именами, но разными типами аргументов
  function sum(a,b:string):string; // Полиморфная функция "суммирования" (соединения) двух строк
  begin
    result := a + b;
  end;
  ...
  function factorial(n:word):Qword; // Вычисление факториала n!
  begin
    if n=0 then result := 1
      else      result := n * factorial(n-1);
  end;

  function summNatural(n:word):Qword; // Сумма натуральных чисел от 1 до n
  begin // Тернарная функция ifThen делает код компактнее, но она зависает при n = 0
    result := 0;
    if n<>0 then result := ifthen(n>1, n + summNatural(n-1), n);
  end;

  // Рекурсия через множественный выбор:
  function factorial2(n:word):Qword; // Вычисление факториала n!
  begin
    case n of
      0..1  : result := 1;
      2..64 : result := n * factorial(n-1);
      else result := 0; // Для запредельных значений возвращаем значение 0
    end;
  end;

  // Рекурсия со вспомогательной вложенной функцией для хранения промежуточного значения:
  function prime(n:word):boolean; // Функция проверки числа на простоту (возвращает true или false)
    function prime_(n,m:word):boolean; // Локальная вспомогательная функция.
    begin                              // Аргумент m хранит число, до которого произведена проверка
       if m*m > n then result := true // Все кандидаты на делители проверены, возвращаем true
         else if n mod m = 0 then result := false // Делитель найден, возвращаем false
           else result := prime_(n,m+1); // Рекурсивная проверка следующего кандидата на делителя
    end;
  begin                   // Запускаем вспомогательную функцию
    result := prime_(n,2) // проверки кандидатов на делители начиная с 2
  end;

  // Оптимизация хвостовой рекурсии - в FPC не реализована, её нужно эмулировать трамлинами:

  // Простая рекурсивная функция определения суммы округлённых квадратных корней чисел от 1 до N.
  // Недостаток этой функции в том, что при N > 0,5 млн рекурсия переполняет стек.
  function sumSqrt(n:qword):qword;
  begin
    if n < 2 then result := n
             else result := sumSqrt(n-1) + round(sqrt(n));
  end;

  // Реализация идентичной функции обычным циклом (для сравнения компактности кода)
  function sumSqrt2(n:qword):qword;
  var i,acc:qword;
  begin
    acc:=0; i:=n;
    repeat
  ...
  // Модифицированная псевдорекурсивная функция с аккумулятором для эмуляции хвостовой рекурсии.
  function sumSqrtTail(var n,acc:qword):boolean;
  begin   // Функция возвращает true если значение вычислено и рекурсию нужно прекратить
    result:=false;
    acc += round(sqrt(n)); // Изменяем аккумулятор (промежуточное значение)
    if n > 0 then          // Условие, при котором нужно продолжать рекурсию
      dec(n) // Меняем значение n на число, для которого будет следующий рекурсивный вызов
      else result:=true    // Подаём сигнал о завершении рекурсии
    { sumSqrtTail(n,acc); - В обычной хвостовой рекурсии здесь должен быть этот рекурсивный вызов,
      который бы мог быть автоматически оптимизирован компилятором (т.е. преобразован в цикл).
      Но текущие версии FPC не умеют делать такую оптимизацию. А значит этот рекурсивный вызов мы
      убираем - вместо него вызов функции sumSqrtTail() будет делать функция Trampoline() }
  end;

  { Универсальная функция-трамплин, выполняющая в цикле переданную ей псевдорекурсивную функцию.
    Использование цикла позволяет вычислять значения для любой глубины рекурсии без риска
    переполнения стека.  Ограничением будет только максимальное значение числа для типа qword. }
  function trampoline(f:TailRecursionType; n:qword):qword;
  var acc:qword;     // Аккумулятор, хранящий промежуточное значение между итерациями
      ready:boolean; // Признак окончания итераций
  begin
    acc := 0;
    repeat               // Этот цикл заменяет хвостовую рекурсию
      ready := f(n,acc); // Функция f() сама изменяет переменные n и acc на каждой итерации
    until ready;         // Как только функция f() сообщила что значение вычислено,
    result := acc;       // возвращаем это значение (оно хранится в аккумуляторе)
  end;
