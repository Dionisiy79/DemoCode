...
implementation { В разделе implementation описываются тела подпрограмм.
  ...
  { Ключевое слово var означает, что аргумент является ссылкой на переданное значение:
    Ключевое слово out - аналог var с той лишь разницей, что из переменная используется только
    для записи, но не для чтения. Это позволяет использовать непроинициализированные переменные
