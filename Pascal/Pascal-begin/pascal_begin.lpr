...
uses sysutils,  // Подключение внешних модулей. Модуль sysutils содержит много популярных функций.
  ...
  {$ifdef UTF8mode}
    ...
    LazFileUtils, // Модуль для работы с файлами и папками в кодировке UTF8
...
var // Объявление имён изменяемых переменных. Производится до начала исполняемого блока.
    ...
    ansistr1 : ansistring;   { Строка-ANSI в текущей однобайтной системной кодировке. Безразмерная -
          ...
          ссылок на себя. Если он равен 0, то строка уничтожается, освобждая память. }
...
// Инициализация переменных
  ...
  {$ifdef UTF8mode} // Строки в UTF-кодировках вопринимают литералы в режиме {$codepage UTF8}
...
begin { ОПЕРАЦИИ }
  ...
  { Битовые операции}
    ...
    inc(i,5); dec(i,5); // Функции увеличения и уменьшения значения указанное число бит
...
begin { ПРИВЕДЕНИЕ ПРОСТЫХ ТИПОВ ПРИ ОПЕРАЦИЯХ }
  ...
  { Явное численное преобразование }
    ...
    // Приведение строки к числу. В случае ошибки приведение генерируется исключение
    numb_2b := StrToInt('123'); // Приведение строки к целому числу
    writeln('Разделитель дробной части : ',DecimalSeparator);
    rnumb_4b := StrToFloat('2,7'); // Приведение строки к вещественному числу
    DecimalSeparator:='.'; // Можно изменить разделитель дробной части
...
begin { МАТЕМАТИЧЕСКИЕ КОНСТАНТЫ И ФУНКЦИИ }
  ...
  rnumb_4b := PI; // Функция PI не иммеет аргументов и возвращает число Пи
  ...
  writeln('Тригонометрические функции');
...
begin { РАБОТА С ФАЙЛАМИ }
   ...
   Erase(typfile1); // Удалить файл, с которым связана файловая переменная

   // Копирование файла
   PCharstr1:='тест-копия2.txt';
   rawstr1:='тест-копия.txt';
   SetCodePage(rawstr1,1251,TRUE); // Меняем кодировку имени файла на 1251
   writeln('Кодировка системы:', Windows.GetACP());

   writeln('Кодировка системы:', DefaultSystemCodePage);
   CopyFile('f1.txt', PChar(rawstr1), true);
   CopyFile('f1.txt', PCharstr1, true);

   // Функции работы с файлами модуля LazFileUtils
   {$ifdef UTF8mode}
