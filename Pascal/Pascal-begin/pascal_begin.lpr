...
var // Объявление имён изменяемых переменных. Производится до начала раздела операторов.
    ...
    // Ссылки на значения простых типов данных

type // Раздел объявления пользовательских типов данных

     { Для стандратных порядковых типов (ordinal types, множество значений которых упорядоченно),
       можно вводить новые ограниченные типы (как подмножества-диаппазоны): }
  ...
  // Перечислимые типы - представляют собой упорядоченные наборы констант, указанные в скобках:
  WeekDays = (Mon, Tue, Wen, Thu, Fri, Sat, Sun); // Новый тип из 7-ми дней недели
  WorkDays = Mon..Fri; // Ограниченный тип на основе предыдущего
  { По умолчанию в перечислимом типе константам соответсвуют номера 0,1,2,3 и т.д.
    Но можно задавать и произвольную нумерацию: }
  MemorySizes = (smallmem=32, midmem=64, bigmem=128); // У константы midmem порядковый номер 64
...
begin { ПРИВЕДЕНИЕ ПРОСТЫХ ТИПОВ ПРИ ОПЕРАЦИЯХ }
  ...
  { Явное численное преобразование }
    ...
    // Получение порядкового номера константы перечислимого типа:
    writeln('У дня недели ', Tue, ' порядковый № ', ord(Tue));
...
begin { ПРОЦЕДУРНОЕ ПРОГРАММИРОВАНИЕ }
  ...
  proc5(640,480); // Передача необязательного (второго) аргумента в подпрограмму
  proc5(640);     // Вызов подпрограммы без второго, необязательного аргумента
