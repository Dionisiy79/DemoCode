...
{ Наиболее важные специальные директивы компилятора, располагаются в начале файла: }
  ...
  {$H+} // Режим, когда тип string - строка неограниченной длины в текущей кодировке компилятора
...
{ Директивы условной компиляции - позволяют компилировать код избирательно: }
  ...
  {$else} // Здесь код, компилирующийся при необъявленном символее. Проверка else не обязательна.
...
const { Объявление имён неизменяемых переменных (констант). Производится до начала исполняемого
        блока указанием значения через знак равенства. Константе автоматически
        присваивается "минимальный" тип данных, достаточный для её представления.}

    NUMB_PI = 3.14; // Для удобства имена констант пишут заглавными буквами, но можно и строчными
...
var // Объявление имён изменяемых переменных. Производится до начала исполняемого блока.
    ...
    PCharstr1 : PChar; { Указатель на массив символов char, как на строку, оканчивающуюся
                        нулевым байтом #0  (Стандарт языка C) }

    textfile1: text;  // Текстовый файл
    binfile1: file of char; // Бинарный файл.
    typfile1: file of real; // Типизированный файл.
  ...
  { Объявления процедур и функций размещается до начала исполняемого блока
    Командами begin и end обозначается начало и конец любого логического(исполняемого) блока.
    Если такой блок указан сразу после объявления имени процедуры или функции,
...
BEGIN // Тело программы - это первая пара begin/end, не являющаяся телом процедуры/функции
  ...
  { Операторы разделяются символом точки с запятой ";"
    ...
    Отключить выполнение кода можно:
      1. Используя директивы условной компиляции $ifdef или $ifndef}
         {$ifndef debug} write('Этот код будет выполнен только при отладке.'); {$endif}  {
...
begin { КОМАНДЫ ВЗАИМОДЕЙСТВИЯ С ПОЛЬЗОВАТЕЛЕМ }
...
begin { РАБОТА С ПЕРЕМЕННЫМИ }
  { Непроинициализированные переменные обычно хранят значение по умолчанию, но стандартами это не
    ...
    Инициализация переменных производится операцией присвоения := (двоеточие и знак равенства) }
...
begin { ОПЕРАЦИИ }
  ...
  { Битовые операции}
    ...
    inc(i,5); dec(i,5); // Функции увеличения и уменьшения значения на указанное число бит
...
begin { ПРИВЕДЕНИЕ ПРОСТЫХ ТИПОВ ПРИ ОПЕРАЦИЯХ }
  ...
  { Явное численное преобразование }
    ...
    numb_2b := round(2.7);  // Округление до целого.  = 3
...
begin { ФУНКЦИИ РАБОТЫ СО СТРОКАМИ }
  ...
  { Доступ к строке как к массиву символов. Работает только:
    ...
    str1 := 'Test string';
    letter1 := str1[5]; // Доступ к 5-му элементу строки (нумерация элементов с 1)
        writeln('5-й символ строки ',str2,' : ',letter1);
    ...
    if (('еж' = 'еж')    and  // Строки равны, если все их символы совпадают
        ('ежик' > 'QW' ) and  // При совпадении символов более длинная строка - больше
...
begin { ПРОЦЕДУРНОЕ ПРОГРАММИРОВАНИЕ }
  ...
  Pascal_Begin_Functions.proc1; // Вызов процедуры proc1 из модуля Pascal_Begin_Functions
    ...
    { С целью последовательного изложения материала все нижеследующие процедуры объявлены в модуле
      Pascal_Begin_Functions. Если Вы уже освоили работу с простыми типами данных, то можете
      приступить к изучению указанного модуля для освоения работы с процедурами и функциями. }

  proc1; // Простейшая процедура
  proc2; // Демонстрация области видимости переменных
  proc3; // Поведение статических переменных
...
begin { ФУНКЦИОНАЛЬНОЕ ПРОГРАММИРОВАНИЕ }
  { Функция - это блок программы, который принимает на вход аргументы и возвращает некое значение
    опредлённого типа. ФП предполагает использование функций без взаимодействия с внешними
...
begin { ОРГАНИЗАЦИЯ ВЕТВЛЕНИЙ В ЛОГИКЕ ПРОГРАММЫ }
  ...
  { БЕЗУСЛОВНЫЕ УПРАВЛЯЮЩИЕ КОМАНДЫ }
    ...
    { Процедуры завершения работы программы }
      Exit;     // Закончить работу программы в штатном режиме
      Halt(17); // Закончить работу программы в штатном режиме, вернув код возврата 17
...
END. { После последнего end (соответсвующему первому begin) ставится точка, а не точка с запятой. }
