...
// ###################################################################################
// #   НАСТРОЙКИ ПОЛЬЗОВАТЕЛЬСКИХ СИМВОЛОВ ДЛЯ КОРРЕКТНОЙ РАБОТЫ С КОДИРОВКОЙ UTF8   #
// ###################################################################################
...
// ####################################################################################
// #   НАЧАЛО БЛОКА ОБЪЯВЛЕНИЯ КОНСТАНТ, ПЕРЕМЕННЫХ И ПОЛЬЗОВАТЕЛЬСКИХ ТИПОВ ДАННЫХ   #
// ####################################################################################
...
const { Ключевое слово CONST начинает объявление имён неизменяемых переменных (констант).
        Производится до начала раздела операторов указанием значения через знак равенства. Константе
        автоматически присваивается "минимальный" тип данных, достаточный для её представления.}
...
var // Ключевое слово CONST начинает объявление имён изменяемых переменных.
    // Производится до начала раздела операторов.
...
// #######################################
    ...
    i, j, k, n :integer; // Объявление нескольких переменных (имена пишутся через запятую).
...
// #########################################
// #   СТАНДАРТНЫЕ СОСТАВНЫE ТИПЫ ДАННЫХ   #
// #########################################
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    arr3 : packed array [1..10] of variant; // Нетипизированный массив требует по 16 байт на эл-т
...
// ~~~~~~~~~~~~~~~~~~
    ...
    { Включение режима {$modeswitch AdvancedRecords} позволяет использовать т.н. расширенные записи,
      ...
      Но это не особо актуально, поскольку такой функционал удобнее реализовывать объектами. }
...
// ~~~~~~~~~~~~~~~~~~~~~
    ...
    pointer1, p1: pointer;  // Безтиповый указатель - может ссылаться на переменную любого типа.
    ...
    { Также "из коробки" доступны типизированные указатели для таких скалярных типов:
      ...
      При этом в списке нет PReal и PValReal.
    }
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  dTableSum1 : array of array of byte; // Двухмерный массив чисел
  AVariant : array of variant;         // Нетипизированный массив

// ####################################
// #   ПОЛЬЗОВАТЕЛЬСКИЕ ТИПЫ ДАННЫХ   #
// ####################################

type // Раздел объявления пользовательских типов данных начинается с ключевого слова TYPE
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// ~~~   ТИПЫ ДЛЯ ДИНАМИЧЕСКИХ МАССИВОВ   ~~~
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  // Если объявить динамический массив как пользовательский тип, то он будут автоматически снабжён
  // конструктором, с помощью которого может оказаться удобнее создавать массивы:
  type digitsRow = array of byte;        // Одномерный динамический массив чисел
       digitsTable = array of digitsRow; // Двухмерный динамический массив чисел
  var dTableSum2: digitsTable;
      dHolo3D: array of array of digitsTable; // 4-мерный массив

// ~~~~~~~~~~~~~~~~~~~~~~~~~~~Ё~~~~~~~~~~~~~
// ~~~   ДИНАМИЧЕСКИЕ СТРУКТУРЫ ДАННЫХ   ~~~
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  { Внутри одного раздела type допустимо объявить новый пользовательский тип, как ссылку на другой
    пользовательский тип данных, который будет описан ниже в этом же разделе. Это позволяет
    создавать серию однотипных элементов, каждый из которых помимо обычных данных может содержать
    ссылки на другие элементы этой серии:}

  // Простейший однонаправленный список:
  type ListLink = ^ListNode;  // Ссылка на узел списка, как на ещё необъявленный тип ListNode
       ListNode = record      // Сразу же объявляем тип для узла списка:
         numb: integer;       // В поле numb хранятся какие-то полезные данные
         next: ListLink;      // В поле next - ссылка на следующий узел списка.
       end;
  var list1, LNode : ListLink; // Нужно 2 переменных - для самого списка и для текущего узла

  // Простейшее бинарное двухсвязное дерево:
  type BTreeLink = ^BTreeNode;  // Ссылка на узел дерева, как на ещё необъявленный тип BTreeNode
       BTreeNode = record       // Тип для узла дерева:
         numb: integer;         // В поле numb хранятся какие-то полезные данные
         L,R,P: BTreeLink;      // Ссылки на левое, правое поддеревья и родительский узел
       end;
      var btree1, BTNode, BTNew : BTreeLink; // Нужно 3 переменных - для дерева и текущих узлов

// ######################################
// #   ОБЪЯВЛЕНИЕ МЕТОК И ПОДПРОГРАММ   #
// ######################################
...
// #################################
// #   НАЧАЛО РАЗДЕЛА ОПЕРАТОРОВ   #  (Нижеследующие блоки можно разворачивать/сворачивать)
// #################################
...
begin { ОБЩИЕ СИНТАКСИЧЕСКИЕ ПРАВИЛА }
  ...
  {$MACRO ON} // В режиме включенных макросов можно создавать именованные наборы символов, которые
              // будут подставленны в код на этапе компиляции
  {$define b:=begin} // Теперь ключевые слова begin и end можно заменять макро-перменными b и e
  {$define e:=end}   // это может пригодится, если нужно сделать код визуально компактнее
  b e; // Сокращённый код, использующий макросы - эквивалентен begin end;
end;
...
begin { СТАТИЧЕСКИЕ МАССИВЫ }
  ...
  // Копирование одного массива в другой возможно простым присвоением, если совпадают их длины и тип
    tablesum2 := tablesum1;
    { Побитно упакованные массивы не совместимы по присвоению с обычно упакованными. Но их можно
       скопировать с помощью  функций pack() и unpack(): }
    pack(Flags1, 3, Flags1bitpacked); { Считывать все элементы начиная с элемента 3 массива Flags1
                          и записывать в самое начало побитно упакованного массива Flags1bitpacked }
    unpack(Flags1bitpacked, Flags1, 5); { Считывать все элементы начиная с самого начала массива
                     Flags1bitpacked и записывать в массив Flags1 в элемент с индексом 5 и далее }
end;
...
begin { ДИНАМИЧЕСКИЕ МАССИВЫ }
  ...
  begin; { СОЗДАНИЕ И ПОЭЛЕМЕНТНЫЙ ДОСТУП К МАССИВУ }
    { Создание динамических массивов }
    // Память для динамического массива выделяется не вручную, а в полуавтоматическом режиме.
    ...
    if (AChar1 = nil) then writeln('Указатель обнулённого массива равен nil'); // true
  ...
  { Использование массивов }
    ...
    // В нетипизированные массивы можно записывать значения разных типов:
    setLength(AVariant,10); AVariant[0]:=7; AVariant[1]:='Строка'; AVariant[2]:=true;
    writeln('Содержимое массива variant: ',AVariant[0],',  ',AVariant[1],', ',AVariant[2]);
    // При подключении модуля uses variants то же самое делает ф-я VarArrayOf([7,'строка',true]);
  ...
  { Перебор элементов массива }
      ...
      for i:=0 to high(AChar1) do     // high(AChar1) = length(AChar1) -1
        AChar1[i] := chr(i+65);
      // Цикл считывания всех без исключения элементов массива:
      for letter1 in AChar1 do
        str1 += letter1;
      writeln('Содержимое массива AChar1: ', str1);

  { Двухмерный массив }
    setLength(dTableSum1,3); // Сначала выделяется память для трёх указателей на строки массива
    // Затем выделяется память для самих строк. При этом можно делать строки разной длины:
    for i:=0 to 2 do setLength(dTableSum1[i],i+1); // Получилась треугольная матрица

    // Если нужно сразу записать значения в массив, то удобно применять пользовательский тип,
    // вызывая из него конструктор create - он объединяет в себе вызов setLength и запись значений
    // в ячейки массива:
    dTableSum2 := digitsTable.create(          // Создать 3 элемента, т.е. 3 одномерных массива
                    digitsRow.create(0,1,2),   // Создать ряд из трёх чисел
                    digitsRow.create(1,2,3),   // Создать второй ряд
                    digitsRow.create(2,3,4)    // и т.д.
    );
    i := dTableSum2[2,2]; // = 4   Обращение к элементу двухмерного массива

  { Многомерный массив }
    // 4-мерный массив хранения пикселей движущейся голограммы
    setLength(dHolo3D,25); // Создание 25 "кадров"
    for i:=0 to 24 do begin
      setLength(dHolo3D[i],10); // Создание 10 трёхмерных массивов
      for j:=0 to 9 do begin
        setLength(dHolo3D[i,j],10); // Создание 10 таблиц в каждом из трёхмерных массивов
        for k:=0 to 9 do begin
          setLength(dHolo3D[i,j,k],10); // Создание 10 рядов в каждой из таблиц
    end; end; end;
    dHolo3D[3,1,7,2] := 80; // Запись значения в созданный 4x-мерный массив;
  end;

  begin { МЕТОДЫ ВСТАВКИ И ВЫЕМКИ ЭЛЕМЕНТОВ ДИНАМИЧЕСКИХ МАССИВОВ С ПЕРЕСТРОЙКОЙ ИНДЕКСА }
    // Вставить массив из трёх элементов в массив AChar1 перед элементом с индексом 6
    insert(['x','y','z'], AChar1, 6);

    // Удалить 2 элемента из массива AChar1 начиная с элемента с индексом 6
    delete(AChar1, 6, 2);

    { Поскольку переиндексация массива возможна только через создание нового массива, то реализацию
      стека и очереди более эффективно делать через динамически создаваемые записи }
  end;

  begin { МЕТОДЫ ОБРАБОТКИ ГРУПП ЭЛЕМЕНТОВ }
  { Копирование участков массива в новый массив без изменения исходного массива }
    // Динамические массивы совместимы по присваиванию, если типы их элементов идентичны. Но для
    // создания копии динамическего массива недостаточно сделать операцию присвоения:
    AChar2 := AChar1; // Скопирован только указатель. Обе переменные указывают на один адрес памяти,
                      // при этом счётчик ссылок на эту область памяти увеличился на единицу.
    AChar1[0] := 'A';
  ...
  { Склейка массивов }
    insert(AChar2, AChar1, length(AChar1)); // Добавить массив AChar2 к массиву AChar1

  { Приведение массива к строке }
    str1 := ''; // Встроенной функции нет - поэтому делается через перебор элементов массива
    for i:=0 to high(dTableSum2) do
      for j in dTableSum2[i] do
        str1 += intToStr(j) + ' ';
    writeln('Содержимое массива dTableSum2: ', str1);

  { Приведение строки к массиву }
    k:=1; // Встроенной функции нет - поэтому всё делается через ручной парсинг строки
    for i:=0 to high(dTableSum2) do
      for j:=0 to high(dTableSum2[i]) do
        begin
          n := pos(' ', str1, k); // Поиск следующего символа пробела в строке, как разделителя
          dTableSum2[i,j] := strToInt(copy(str1, k, n-k)); // Парсинг числа из букв между пробелами
          k := n+1; // Перемещаем указатель, до какого символа строка уже просмотрена
        end;

  { Сортировка массива }
    // Встроенной функции нет, поэтому сделать можно через ручную сортировку, напр. методом пузырька
    for i:=0 to high(AChar1)-1 do
      for j:=0 to high(AChar1)-1-i do
        if AChar1[j] > AChar1[j+1] then
          begin // Если предыдущий элемент больше следующего, то меняем элементы местами
            letter1 := AChar1[j]; AChar1[j] := AChar1[j+1];  AChar1[j+1] := letter1;
          end;

  { Инверсия массива (изменение порядка элементов на обратный) }
    for j:=0 to high(AChar1) div 2 do // Встроенной функции нет, поэтому делаем обычным циклом
    begin // Меняем противоположные элементы местами:
      letter1 := AChar1[j]; AChar1[j] := AChar1[high(AChar1)-j]; AChar1[high(AChar1)-j] := letter1;
    end;

  { Поиск по массиву }
    for j:=0 to high(AChar1) do // Встроенной функции нет, поэтому делаем обычным циклом
      if AChar1[j]='F' then break;
    writeln('Символ F найден в позиции ', j);

  end;

end;

begin { СПИСКИ, ОЧЕРЕДИ, СТЕКИ }

  { Имея описанный пользователем тип для списка можно создавать и обрабатывать динамически
    изменяемую структуру данных. В идеале для всех операций создаются соответсвующие функции,
    но в целях краткой демонстрации мы ограничимся только компактными блоками кода.
    Также в целях упрощения подразумевается, что созданные списки не пусты (т.е. не равны nil) }

  { Создание списка }
    new(list1); // Выделяем память для головного узла списка
    list1^.numb:=5; list1^.next:=nil; // Записываем данные. Т.к. узел всего один, то поле next = nil

  { Добавление элементов в список }

    // Реализация стека - добавление узла в голову списка:
      new(LNode);                            // Создаём новый узел
      LNode^.numb := 9; LNode^.next := list1; // Он ссылается на голову списка
      list1 := LNode; // Делаем адрес этого нового узла головой списка.

    // Реализация очереди - добавление узла в хвост непустого списка
      LNode := list1; // Делаем текущим головной узел. И далее в цикле доходим до хвоста:
      while LNode^.next <> nil do LNode := LNode^.next;
      new(LNode^.next); // Добавляем к хвосту ещё один хвост.
      LNode := LNode^.next; // Записываем информацию в созданный узел:
      LNode^.numb := 2; LNode^.next := nil;

    // Добавление элемента в середину непустого списка
      LNode := list1; n:=1; // n - номер нужного существующего узла
      for i:=1 to n do LNode := LNode^.next; // Переходим к нужному узлу n
      pointer1 := LNode^.next; // Сохраняем ссылку на узел n+1 во временную переменную
      new(LNode^.next); LNode := LNode^.next; // Вставляем новый узел n+1 после узла n
      LNode^.numb := 3; LNode^.next := pointer1; // Записываем в узел данные и ссылку узел n+2

  { Чтение всех узлов списка }
    LNode := list1; str1 := inttostr(LNode^.numb) + ' '; // Прочитали голову
    repeat // В цикле читаем последующие элементы, доходя до хвоста
      LNode := LNode^.next;
      str1 += inttostr(LNode^.numb) + ' ';
    until LNode^.next = nil;
    writeln('В списке list1 хранятся числа: ', str1);

  { Вынимание элементов из списка}

    // Реализация стека и очереди - вынимание узла из головы непустого списка:
      LNode:=list1;                 // Помещаем указатель на голову списка
      k := list1^.numb;             // Считываем данные
      list1 := list1^.next;         // Делаем головным второй элемент
      dispose(LNode); LNode := nil; // Удаляем первый элемент
      writeln('Из стека/очереди вынуто значение ', k);

    // Реализация двухсторонней очереди (деки) - вынимание узла из хвоста списка:
      LNode := list1; // Делаем текущим головной узел и в цикле доходим до предпоследнего узла
      if list1 <> nil then
        if list1^.next <> nil then // Проверяем, содержит ли список более одного узла
          begin
            while LNode^.next^.next <> nil do LNode := LNode^.next;
            k := LNode^.next^.numb;                   // Считываем данные из последнего узла
            dispose(LNode^.next); LNode^.next := nil; // Удаляем последний узел
          end
        else
          begin
            k := list1^.numb;     // Считываем данные из единственного узла
            dispose(list1); list1 := nil;  // Удаляем единственный узел
          end;
      writeln('Из деки вынуто значение ', k);

      // Вынимание узла из середины списка
        LNode := list1; n:=1;   // n - номер нужного существующего узла
        for i:=1 to n-1 do LNode := LNode^.next; // Переходим к узлу, стоящему перед узлом n
        k := LNode^.next^.numb; // Считываем значение узла n
        dispose(LNode^.next);   // Удаляем узел n
        LNode^.next := LNode^.next^.next; // У узла n-1 теперь ссылка на узел n+1
        writeln('Из середины списка вынуто значение ', k);
end;

begin { БИНАРНЫЕ ДЕРЕВЬЯ }
  { Создание дерева }
    new(btree1); // Выделяем память для корня дерева
    btree1^.numb:=50; btree1^.L:=nil; btree1^.R:=nil; btree1^.P:=nil; // Записываем данные.

  { Добавление нового узла в непустое бинарное дерево поиска }
    // В дереве поиска значение каждого узла больше/меньше значений в левом/правом поддереве
    for i := 1 to 20 do // Заполним дерево случайными числами
    begin
      k:=random(99);   // Добавляемое значение
      new(BTNew); BTNew^.numb:=k; BTNew^.L:=nil; BTNew^.R:=nil; // Создаём новый узёл вне дерева
      BTNode := btree1; // Начинаем поиск с корня дерева
      repeat // Цикл спуска по дереву в поиске подходящего места для нового узла-листа
        BTNew^.P:=BTNode;
        with BTNode^ do begin
          if      k < numb then j:=1 // Намечаем спуск в левую ветку
          else if k > numb then j:=2 // либо в правую
               else break; // Одинаковые узлы недопустимы - выход из цикла, ничего не делаем
          if j=1 then p1:=L else p1:=R; // Запоминаем адрес выбранной ветки в указатель p1
          if p1 = nil then                                        // Если ветка пустая, то
            begin if j=1 then L:=BTNew else R:=BTNew; break; end; // прикрепляем сюда лист и выход
        end;
        BTNode:=p1; // Иначе спускаемся в выбранную ветку
      until false;
    end;

  { Обход бинарного дерева }
    // Классически деревья удобнее обходить через рекурсию. Но чтобы не плодить функции
    // и параллельно показать методику избегания переполнения стека, сделаем это через цикл.
    // Для этого дерево должно быть двухсвязное - т.е. узлы имеют ссылку на своего родителя.

    BTNode := btree1; // Стартуем с корневого узла
    j:=1;  { Флажок - откуда пришли в узел:  1 - от родителя, 2/3 - от левого/правого потомка }
    writeln('Содержимое двоичного дерева поиска:');
    repeat
      if j=1 then write(BTNode^.numb, ' '); // Выводим значение, если впервые посещаем узел
      case j of                             // Смотрим откуда мы попали в текущий узел
        1 : if BTNode^.L = nil then j:=2 // Если слева потомка нет, то считаем что вернулись из него
            else BTNode := BTNode^.L;    // иначе спускаемся в левое поддерево.
        2 : if BTNode^.R = nil then j:=3 // Если справа потомка нет,то считаем что вернулись из него
            else begin BTNode := BTNode^.R; j:=1; end; // иначе спускаемся в правое поддерево.
        3 : if BTNode^.P = nil then break  // Если вернулись к корню справа, то конец цикла
            else begin // Иначе движемся вверх, запоминая откуда пришли - слева или справа:
                   if BTNode = BTNode^.P^.L then j:=2 else j:=3;
                   BTNode := BTNode^.P;
                 end;
      end;
    until false; writeln;
