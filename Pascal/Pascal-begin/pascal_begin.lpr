...
{ Директивы условной компиляции - позволяют компилировать код избирательно }
  ...
  // Использование символов для условной компиляции:
  {$ifdef Sym1} // Блок, который компилируется, если указанный символ объявлен
    // Код, выполняющийся при объявленном символе Sym1
  {$else}
    // Код, выполняющийся в противоположном случае
    {$error В вашей системе код нельзя скомпилировать!} // Остановка компиляции с генерацией ошибки
  {$endif}

  {$ifndef Sym1} // Блок, который компилируется, если указанный символ НЕ объявлен
    // Код, выполняющийся при НЕ объявленном символе Sym1
  {$else}
    // Код, выполняющийся в противоположном случае
  {$endif}
...
uses sysutils, Math, // Подключение внешних модулей
  ...
  {$ifdef windows} windows,   {$endif} // Модуль со специфичными функциями Windows
    ...
    LazUTF8; // Модуль для корректной работы терминала в кодировке UTF8
...
{$ifdef Win64} // Блок, который компилируется, если указанный символ объявлен
  // Код, выполняющийся при объявленном символе Sym1
{$else}
  // Код, выполняющийся в противоположном случае
{$endif}
