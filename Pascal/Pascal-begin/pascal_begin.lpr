...
const { Объявление имён неизменяемых переменных (констант). Производится
    ...
    ConstStr2: AnsiString = 'Строка'; { Если явно указать тип строковой константы, то кодировка
        будет соответсвовать указанному типу. В данном случае вместо UTF-16 получаем UTF-8 }
    {$J+}
...
shortstr2 := 'а1бвгдеёзжиклмнопрстуфхцчшщэюя';  // Строка будет обрезана до 10 символов
ansistr1 := 'Строка ANSI Ψ';   // Строка-ANSI неограниченного размера
widestr1 := 'Строка в двухбайтной кодировке  UTF-16';
unistr1 := 'Строка - Unicodestring Ψ';
utf8str1 := 'Строка - UTF8string Ψ';
rawstr1 := 'Строка без указания кодировки Ψ';
PCharstr1 := 'Строка - массив символов Char Ψ';
