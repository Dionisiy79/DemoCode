...
var // Объявление имён изменяемых переменных. Производится до начала исполняемого блока.
    ...
    i, j, n, factorial1 :integer; // Объявление несколько переменных (имена пишутся через запятую).
...
// Инициализация переменных производится операцией присвоения := (двоеточие и знак равенства):
  ...
  letter1 := 'W';            // Непроинициализированная переменная хранит латинскую букву 'a'
    writeln(letter1);
...
begin { ФУНКЦИОНАЛЬНОЕ ПРОГРАММИРОВАНИЕ }
  ...
  str1 := Pascal_Begin_Functions.func1();
  writeln('Функция func1 вернула значение: ', str1);
