...
{ Многострочный комментарий - обрамляется фигурными скобками.
  Этот файл сохранен в кодировке DOS-866. Для корректного открытия в Lazarus IDE следует в окне редактора кода выбрать Параметры файла->Кодировка->CP866.
  ( ▌ЄюЄ Їрщы ёюїЁрэхэ т ъюфшЁютъх DOS-866. ─ы  ъюЁЁхъЄэюую юЄъЁ√Єш  т Lazarus IDE ёыхфєхЄ т юъэх ЁхфръЄюЁр ъюфр т√сЁрЄ№ ╧рЁрьхЄЁ√ Їрщыр->╩юфшЁютър->CP866.)
}
...
// Объявление имён и типов переменных.
  { Имя переменной должно начинаться с латинской буквы или символа подчеркивания,
    может также содержать цифры. Длина имени от 1 до 127 символов. Написание не чувствительно к регистру.
  }
const  // Объявление имён неизменяемых переменных (констант). Производится до начала исполняемого блока. Константе автоматически присваивается "минимальный" тип данных, достаточный для её представления.
    PI = 3.14;      // Для удобства имена констант пишут заглавными буквами, но строчными тоже допустимо
    HI = 'Привет';
    realnumb:Double = 789; // Т.н. типизированная константа, которая по сути является переменной (её можно менять по ходу программы). Её тип задаётся через двоеточие
...
var  // Объявление имён изменяемых переменных. Производится до начала исполняемого блока.
    ...
    // Вещественные числа
    rnumb_4b : Single;    // 4 байта ( 2,9*10^-39   .. 1,7*10^38   ) - с плавающей точкой
    ...
    rnumb_8bf : Currency; // 8 байт ( -922337203685477.5808 .. 922337203685477.5807 ) - с фиксированной точкой
    ...
    arr1 : array [1..100] of integer; // Нумерация элементов может начинаться не с нуля,а с любого целого числа.
    arr3d : array [1941..1945,1..12,1..31] of integer; // Трёхмерный массив - для хранения значений в ячейках с индексами указывающими на год, месяц, число.

  label mimimi;  // Объявление метки с именем mimimi
...
{ Комманды взаимодействия с пользователем }
  ...
  writeln('Нажмите любую клавишу:'); readkey;     // Ожидание нажатия любой клавиши. Требует uses Crt
  write('Нажмите ENTER:'); readln;      // Ожидание нажатия клавиши ENTER (ввода строки)
  }

// Инициализация переменных
  numb_4b := 1234567890;     // Непроинициализированная переменная обычно хранит значение 0
  numb_1b := 12345;          // Число больше допустимого 127, поэтому сохранится неправильное значение (обрезанное до 8 бит)
...
// Использование переменных
  ...
  numb_2b := numb_1b;
  write(rnumb_6b);
  writeln(' ',letter1,str1,bool2); // В функции вывода можно перечислять выводимые аргументы через запятую.
  rnumb_8b := rnumb_4b / numb_4b; // Использование значений переменных в арифметических выражениях
//  write(rnumb_8b * 100);

{ ОПЕРАЦИИ }
  numb_1b := ( 7 + 2 - 3 ) * 4; // Сложение вычитание умножение для любых типов чисел.
  numb_1b_p := 1 - numb_1b; // Результирующее значение не проверяется на предмет попадания в допустимый диапазон:
  writeln('В переменной типа BYTE отрицательное значение ', 1 - numb_1b,' сохранилось как ', numb_1b_p);
  numb_2b := 2147483647 + 2;  writeln('В переменной типа INTEGER 33-битное значение ', 2147483647 + 2,' сохранилось как ',numb_2b);

  numb_2b := 31 div 8; // Целочисленное деление
  numb_2b := 31 mod 8; // Остаток от деления
  rnumb_6b := 31 / 8;  // Вещественное деление

  numb_4b := sqr(3);        // Квадрат числа. Для целых чисел результат функции SQR можно сохранить в целочисленную переменную.
  numb_4b := 3**5;          // Возведение в произвольную степень. Если операнды целочисленные, то результат можно сохранить в целочисленную переменную.
  rnumb_8b := power(2,3);   // Аналогичная функция POWER даже для целых чисел возвращает вещественный результат.
  rnumb_8b := 2**3.5;       // Возведение в дробную степень
  rnumb_8b := power(2,3.5);

  { Унарные операторы }
  i := - numb_4b; // Унарный оператор смены знака.
  i := not i; // Побитовая инверсия - вычитание из -1. Т.е. смена знака и одновременный декремент. Полезна в связке с операцией смены знака.
  i := -not i; // Инкремент. Унарный аналог i+1
  i := not- i; // Декремент. Унарный аналог i-1
  i := -not 2 * 6; // Результат равен 18, а не 13, поскольку у унарных операций приоритет выше чем у операций умножения и деления.
  ...
  i := i << 3; i := i shl 2; // Побитовый сдвиг влево на указанное число бит с сохранением знака (умножение на степень двойки )
  i := i >> 2; i := i shr 3; // Побитовый сдвиг вправо на указанное число бит с сохранением знака (деление на степень двойки)
  ...
  i := i xor 5; // Побитовое ИСКЛЮЧАЮЩЕЕ ИЛИ (XOR). Используется для смены значений нужных битов на противоположные.
...
{ ИНИЦИАЛИЗАЦИЯ И ИСПОЛЬЗОВАНИЕ СОСТАВНЫХ НАБОРОВ ДАННЫХ }
  ...
  arr1[6] := arr1[5] + 2; // Элемент массива можно использовать в выражениях как обычную переменную указанного типа.
...
{ ОРГАНИЗАЦИЯ ВЕТВЛЕНИЙ В ЛОГИКЕ ПРОГРАММЫ }
  ...
  if bool1 then str1:='Yes' // Перед ключевым словом ELSE точка с запятой не ставится
    else str1:='No';
    // Запись в несколько строк:
  if bool1 then
    begin
  ...
  case numb_4b of    // Значение переменной далее сравнивается с образцами:
    0        : j:=0; // Можно указать конкретное значение при совпадении с которым выполняются операции после двоеточия.
    1..100   : j:=2; // Можно указать диаппазон значений.
    647, 729 : j:=3; // Можно указать несколько значений через запятую.
    else j:=-1;      // Значение по умолчанию, если совпадений не найдено. Эта строка не обязательна.
  end;
...
{ ЦИКЛЫ }
  // Цикл со счётчиком
  for i:=1 to 9 do write(i); // На каждой итерации счётчик увеличивается на 1 в порядке возрастания.
  writeln;
  factorial1 := 1;
  for i:=2 to 5 do // Если в теле цикла несколько операторов, то они заключаются в логический блок BEGIN-END
    begin
  ...
  writeln('После выполнения циклов значения счётчиков сохраняются: i = ',i,' и j = ',j);
  ...
  repeat
    i := i div 3;
    write(i,' '); // Можно использовать несколько операторров без заключения их в блок begin-end
  ...
  repeat
    ...
    write(i,' ');
  until false; // При таком условии цикл будет повторяться вечно, если его не прервать
  ...
  // Для надёжности лучше использовать переменную строкового типа, потому как если в численную переменную пользователь по невнимательности введёт буквы, то программа зависнет.
  writeln(str1);
...
end. // Коммандой end обозначается конец логического блока программы.
