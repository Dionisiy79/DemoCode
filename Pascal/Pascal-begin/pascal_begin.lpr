...
var // Объявление имён изменяемых переменных. Производится до начала раздела операторов.
  ...
  { МНОЖЕСТВА (НАБОРЫ) }
    { Множество в FPC - набор из однотипных элементов-констант любого перечислимого типа, являющимся
      ...
      digits_set1 : set of 0..9; // Множество, способное хранить только цифры (базовый тип - byte)
      digits_char_set1 : set of '0'..'9'; // Может хранить только символы цифр (базовый тип - char)
...
begin { МНОЖЕСТВА (НАБОРЫ) }
  ...
  { Инициализация и использование множеств }
    ...
    char_set1 := digits_char_set1; // При совпадении типов можно сделать копию другого множества.
  ...
  { Операции над множествами }
    // Операция принадлежности IN проверяет, есть ли в множестве элемент с указанным значением:
      ...
      if letter1 in digits_char_set1 then
        writeln(letter1,' является цифрой.');
  ...
  { Сравнение множеств }
    ...
    if (([1..9] = [1..9]) and // Множества равны, если имеют одинаковые наборы элементов
       ([1..9] <> [1..8]) and // Множества не равны, если не имеют одинаковых наборов элементов
  ...
  { Перебор всех элементов множества}
    str1:='';
    for letter1 in char_set2 do
      str1 += letter1;
    writeln('Множество char_set2 содержит символы: ', str1);
