/* В этом файле преставлен код, демонcтрирующий и описывающий основы и важные особенности языка 
   программирования JavaScript в стандарте EcmaScript 5. Запускается этот код на странице 
   js-begin.html, а весь вывод результатов своей работы осуществляет в консоль браузера. 
...
"use strict"; /* Этот строковый литерал, если стоит первым, переводит интерпретатор в режим 
                 повсеместно поддерживаемого стандарта ES5+. При этом теряется совместимость с IE9- 
                 и скриптами, написанными в совсем древних стандартах.  
                    https://learn.javascript.ru/strict-mode 
                    https://habr.com/ru/articles/791978/  
              */
{
  {/* ОБЩИЕ ЛЕКСИЧЕСКИЕ ТЕРМИНЫ И СИНТАКСИЧЕСКИЕ ПРАВИЛА
  
    - Инструкция - это фрагмент кода, который выполняет действие.
    - Объявления - фрагмент кода, декларирующего новые переменные и функции.
    - Выражение - фрагмент кода который возвращает значение.
      Элементарными выражениями являются: 
        - Литералы (наборы символов, обозначающих конкретное значение, например 
          45 (число), "45" строка, [2,3] (массив значений) и т.д.
        - Переменные, возвращающие в выражение либо хранящееся в них значение либо ссылку на что-то.
    - Операция - выражение, совершающее действие над выражениями (операндами операции)
    - Оператор - набор символов, используемый для выполнения операций над операндами.
  
    - Инструкции в JS обычно заканчиваются символом точки с запятой `;`
    - Если в интрукции символ `;` пропущен, но есть перевод строки, то интерпретатор может его 
          автоматически вставить, но надеяться на это не стоит:
            https://learn.javascript.ru/structure#komandy
    - Блок инструкций выделяется фигурными скобками {}
    */ 
    
    /* В JS нет дирректив условной компиляции, поэтому отключить выполнение кода, можно либо 
       закомментировав его, либо поместив внутрь условного блока `if(){  }` */
       if(0){ "Невыполнящийся код" } // В коде не должно быть ошибок (он всё равно компилируется)
            // Для включения выполнения нужно 0 заменить на 1 или другое логическое выражение.
  }
}
...
{// КОМАНДЫ ВЗАИМОДЕЙСТВИЯ С ПОЛЬЗОВАТЕЛЕМ //
  ...
  { /* Вывод текстовых строк  
            ...
            Прежде начала изучения самых основ языка программирования следует знать, какими 
            средствами можно просматривать результат работы программы и вводить свои данные.
            В текущих блоках инструкций перечислены предназначенные для этого функции, 
            методы объектов и прочие непростые конструкции, активно применяемые в последующем 
            демонстрационном коде (наряду с поробным описанием принципов работы с такими сложными
            конструкциями). А на данном этапе достаточно относится к этому коду, как к сборнику 
            примеров так сказать "для затравки".
    */
        console.log('Hello World!');  // Вывод строки в консоль (с переносом строки)
        console.log('а','б','в');     // Можно вывести несколько строк, указав их через запятую
        
        console.group("Демо-группа строк 1");  // Объявление начала группы строк в консоли
            // Объявление не простой, а свернутой группы строк, к тому же внутри первой группы:
        ...
        document.write('Hello world! ');  /* Вывод строки в текущее место веб-страницы 
  ...
  { /* Пауза на 3 секунды
       Будет отложено выполнение только блока операций функции func(). 
...
{// ХРАНЕНИЕ И ОБРАБОТКА ДАННЫХ В ОПЕРАТИВНОЙ ПАМЯТИ. //
  ...
  {/* РАБОТА С ПЕРЕМЕННЫМИ 
      ...
      Регистр имеет значение. В стандарте JS имя переменной называется идентификатором.
        https://learn.javascript.ru/variables
    */
  console.groupCollapsed("Хранение и обработка данных в оперативной памяти");
    console.groupCollapsed("Переменные"); // "Collapsed" указывает, что группа строк свёрнутая

    // ОБЪЯВЛЕНИЕ И ИНИЦИАЛИЗАЦИЯ ПЕРЕМЕННЫХ //
    
        /* В ES5 новая переменная объявляется с помощью ключевого слова var в любом месте кода.
           Это допустимо потому, что все объявления компилятор как бы виртуально переносит в самое 
           начало кода программы, либо в самое начало функции, внутри которой объявлена переменная. 
           Это явление называется всплытием объявлений переменных.
        */
        var str1; // Создан идентификатор str1
        var str1; /* Если переменная объявлена ранее, то при повторном объявлении директива var 
                      будет просто проигнорирована и ошибки не возникнет. */
        str1 = "строка"; /* Выражение, являющееся одновременно операцией присваивания. 
                            Вычисляется справа налево:
                                - Берётся значение строкового литерала "строка" 
                                - Через оператор присваивания `=` помещается в переменную str1
                                - Возвращается новое значение переменной str1 */
        
        /* Одиночные литералы и переменные, будучи выражениями, могут быть записаны как инструкция, 
           выполняющая действие по возврату значения */
            "строка 2";   // Возвращено значение "строка 2" 
            str1;         // Возвращено значение "строка"

        str2 = "Ещё строка"; // Благодаря всплытию объявлений допустимо разместить код, использующий
                             // переменную до строки кода с её объявлением.
        var str2; // Эта строка кода виртуально всплывёт выше предыдущей и всё будет работать.
        
        /* Объявление и инициализацию можно записать в одну строку. При этом компилятор разделит
           эту единую строку на две (строка объявления и строка инициализации) и для строки 
           объявления применит всплытие.
        */
        var numb1 = 213; /* Заменится на:
                                var numb1; 
                                numb1 = 213;
                         */
    ...
    // АВТОМАТИЧЕСКОЕ УПРАВЛЕНИЕ ПАМЯТЬЮ //
        ...
        /* В JavaScript нет чёткого разделения на значимые и ссылочные типы данных. Многое зависит
           от реализации хранения данных в конкретном движке. Так, например, самый популярный движок
           V8 многие примитивные типы эмулирует объектами в целях оптимизации производительности.
           Поэтому если говорится о типе, как о примитивном, то имеется в виду логическое, можно 
           сказать философское понимание термина, а не то, как этот тип реально хранится в памяти.
           
           Также важно знать, что в JavaScript реализовано полностью автоматическое управление 
           памятью, включая сборку мусора. Это означает что программисту вообще не нужно вручную 
           выделять память для сложных структур (например объектов). Исключение составляют бинарные
           массивы, которые, впрочем, на практике используются довольно редко. 
           
           Благодаря сборке мусора программисту также не надо заботиться об ручном освобождении
           памяти - неиспользуемые данные удаляются автоматически сборщиком мусора, если этот 
           сборщик обнаруживает, что на созданный объект больше не ссылается ни один из существующих
           идентификаторов. Это конечно не означает, что можно бездумно порождать миллионы 
           переменных. Речь о том, что программисту достаточно записать в переменную любое другое 
           значение (например null), чтобы она перестала ссылаться на выделенную для неё ранее 
           область памяти. Если область памяти стала недостижима с помощью объявленных переменных, 
           то она будет очищена на одной из последующих итераций сборки мусора.
    ...
    // БАЗОВЫЕ ТИПЫ ДАННЫХ //
        ...
        numb1 = 456;      // Для чисел используется стандарт IEEE-754 64-битный с плавающей точкой
        ...
        str1 = " \\ ";  // Для сохранения в строку экранирующего символа его нужно написать дважды 
    ...
    // ИСПОЛЬЗОВАНИЕ ПЕРЕМЕННЫХ //
        ...
        // Проверка, инициализирована ли переменная. Работает и для необъявленных переменных
            ...
            console.log('str1 ===',str1); // Удобно выводить значение вместе с именем переменной
            /* Также удобно выводить значения с именами, заключив их в фигурные скобки в этом случае 
               на лету создаётся объект, который красиво отображается в консоли брауера: */
        ...
        str1 = 5; /* В str1 помещена новая ссылка (на число 5), поэтому память, хранящая текст,
             к которому раньше был доступ из переменной str1, будет позже очищена сборкой мусора */
...
{// ПОДПРОГРАММЫ (ФУНКЦИИ) //
  ...
  {/* ПРОЦЕДУРНОЕ ПРОГРАММИРОВАНИЕ 
    ...
    */
    console.groupCollapsed("Подпрограммы (функции)");
    ...
    /* Функция - это блок программы, который принимает на вход аргументы 
      ...
      Процедура - блок программы, которому могут передаваться или не передаваться на вход аргументы, 
                ...
                побочные действия (ввод/вывод, изменение значений внешних переменных и т.д.).
      ...
      Объявление функции в режиме простой декларации (Function Declaration) допустимо в любом месте 
      кода. В некоторых старых браузерах strict-режим накладывает ограничение объявлять функцию 
      только на верхнем уровне кода или верхнем уровне внутри другой функции. Т.е. объявление 
      функции нельзя помещать в блок инструкций {}.
      
      Другой режим объявления функции - это функциональное выражение (Function Expresion), когда 
      функция при объявлении записывается в переменную (хранящую ссылку на созданную функцию). 
      В таком режиме функции можно помещать в блоки инструкций `{}`. 
      
      В связи с этими особенностями весь дальнейший код обёрнут в блок функционального выражения.

      В ES5 все функции создаются с помощью ключевого слова function() после которого размещается
      блок операций, являющийся телом функции.
    */
   
    var f01 = function() { // Функциональное выражение, помещающее функцию в переменную f01
  
        function proc1() { // Простая декларация функции требует указать её имя после слова function
    ...
    // Вызов функций без передачи в неё аргументов: //
    
        proc1(); // Вызов функции с именем proc1 
        proc3(); /* В режиме Function Declaration вызов функции можно делать до объявления функции 
                    поскольку простые декларации всплывают аналогично объявлениям var-переменных.
                    При этом всплывает как имя функции, так и её тело с встроенными свойствами 
                 */
                 
        var proc1b = function(){   /* Функция в режиме Function Expression - создаётся в момент
                                      вычисления выражения (т.е. не всплывает).  */
    ...
    // ОБЛАСТЬ ВИДИМОСТИ ПЕРЕМЕННЫХ //
        ...
        /*  В ES5 реализована только функциональная видимость переменных (в отличие от ES6, где есть
            ещё и блочная видимость). Это означает, что переменная объявленная внутри функции, будет
            невидима снаружи функции. При этом изнутри самой функции видны внешние переменные. Чтобы
            правильно обращаться со внешними переменными важно понимать, как именно организован 
            доступ к ним под капотом JS.
            
            Технически функции в JS являются специальными объектами (т.н. функциональные объекты), 
            в которых помимо самого кода функции хранятся вспомогательные данные. В частности у 
            функции есть скрытое (недоступное из программы) свойство [[Scope]] (в переводе означает 
            "область"). В этом свойстве хранится ссылка на объект или функцию, внутри блока 
            инструкций которого она была объявлена. Т.е. на объект, окружающий данную функцию.

            Тут важно отметить, что поскольку эта ссылка берётся на основании текста программы, то
            название такого внешнего окружения функции другими объектами называется "лексическим 
            окружением" (или "статическим окружением"). В ряде других языков программирования 
            используется иной тип окружения - динамическое, при котором родителем для функции 
            выступает функция или подпрограмма, из которой она была вызвана.
            
            От типа окружения принципиально зависит то, какие в итоге значения переменных окажутся
            в работающей функции. Т.е. с каким из объектов функция будет связана и что будет в 
            случае, если родительский объект перестал существовать до завершения работы функции. 
                Есть всего три вида связываний:
                1. Глубокое связывание (deep binding) - функция выполняется в контексте своего 
                   описания в коде программы (используется лексическое окружение).
                2. Поверхностное связывание (shallow binding) - функция выполняется в контексте 
                   своего вызова (используется динамическое окружение).
                3. Специальное связывание (ad hoc binding) - создаётся копия контекста выполнения, 
                   каким он был на момент запуска функции, и уже из него функция берёт значения. 
            
            В JavaScript реализован 3-й вид: специальное связывание с применением лексического 
            замыкания (closure). Тема замыканий раскрыта ниже в разделе "Функциональное 
            программирование". С одной стороны создание замыканий ухудшает производительность, но 
            с другой повышает надёжность программ и, в частности, позволяет полностью решить т.н. 
            "проблему фунарга" (функционального аргумента) - когда функция используется как аргумент 
            в разных других функциях или является возвращаемым результатом работы другой функции
            (что можжет вызывать целый ряд неоднозначностей и ошибок выполнения).
            
            В JavaScript специальное связывание реализовано следующим образом. В момент запуска 
            функции у неё создаётся специальный объект [[Execution context]], который помещается 
            в стек исполнения и тем самым становится текущим контекстом выполнения (после завершения 
            работы функции объект удаляется из стека, делая текущим предыдущий контекст).
            
            В этот объект помещается ссылка на другой спец.объект [[LexicalEnvironment]] (т.е. 
            "лексическое окружение"), который тоже создаётся и в нём сохраняется ссылка [[outer]] 
            на внешний [[LexicalEnvironment]], а также запись [[envRec]], в которой хранятся все 
            локальные переменные функции. Чаще всего в поле [[outer]] просто копируется значение 
            [[Scope]] запущенной на выполнение функции.
            
            Иерархия ссылок между этими всеми объектами выглядит так:
                - Объект [[Execution context]]
                    - Объект [[LexicalEnvironment]] 
                        - Поле [[outer]] - ссылается на внешний [[LexicalEnvironment]]
                        - Объект [[envRec]]
                            - Список со значениями переменных
                    - Неизменяемый объект [[VariableEnvironment]], хранящий только var-переменные.
                    - Поле ThisBinding - текущее значение this (о нём будет речь в разделе ООП)
            
            В итоге получается, что для работающей на текущий момент функции в стеке исполнения есть
            вся информация об её переменных и её лексическом окружении. Если какая-то переменная не
            найдена в [[envRec]] самой функции, то происходит переход по ссылке [[outer]] в 
            родительский [[LexicalEnvironment]] и т.д. по цепочке родителей, пока не будет либо 
            найдена сама переменная, либо достигнут объект window, у которого [[outer]]==null.
            
            И даже если функция закончит работу и соответсвующий ей объект [[Execution context]]
            удалится из стека, но при этом функция породила другие функции, ссылающиеся на её 
            [[LexicalEnvironment]] через поле [[outer]], то этот объект [[LexicalEnvironment]] будет
            оставаться в памяти до тех пор, пока не отработают все ссылающиеся на него дочерние 
            функции. Это и есть то самое замыкание (closure), благодаря которому у вызванной функции
            всегда есть как бы закрытый (замкнутый) сундучок со всеми нужными ей данными.
                
                https://learn.javascript.ru/closures
                https://jsflow.org/docs/scope-fundamentals/
                https://jsflow.org/docs/lex-env/
                https://jsflow.org/docs/exec-context-fundamentals/
                https://jsflow.org/docs/create-exec-phase/
    ...
    // ЛОКАЛЬНЫЕ ПЕРЕМЕННЫЕ ФУНКЦИИ //
        ...
        function proc2() { 
            str1 = "Новое значение"; /* Из тела функции можно изменять внешние переменные. 
                Если внешняя переменная не существует, то в режиме Strict это вызовет ошибку. */
            var str2 = "Локальное значение";  /* Директивой var можно объявить локальную переменную, 
                                                 даже если её имя совпадает с внешней. 
                                При этом в теле функции обрабатывается локальное значение: */      
            console.log("Внутри функции str2 == " + str2); 
        }
        str1 = str2 = "Внешнее значение";  // Внешние переменные
        proc2(); // Функция изменила внешнюю переменную str1 (плохое поведение)
         console.log("Теперь str1 == " + str1); 
            
        // Вне тела функции остаётся неизменным внешнее значение переменной str2:
        console.log("Вне тела функции str2 == " + str2); 
        
        // Демонстрация того, что область видимости лексическая, а не динамическая:
        
        str1 = "Внешняя переменная (в лексическом окружении)";
        function proc2_log() { 
            console.log("В окружении оказалась: " + str1); //  ???
        }
        function proc2_call_log() { 
            var str1="Переменная внутри вызывающей функции (в динамическом окружении)";
            proc2_log(); // Внутри функции вызывается внешняя функция 
        }
        proc2_call_log(); // "Внешнее значение" (хранится в [[LexicalEnvironment]] родителя)
        // А вот для динамического окружения вывелась бы переменная внутри вызывающей функции

    // ДОСТУП КО ВСЕМ АРГУМЕНТАМ, ПЕРЕДАННЫМ В ФУНКЦИЮ // 
        
        /* В любую функцию можно передавать различные значения (аргументы).
           Доступ в теле функции к любым полученным аргументам (в т.ч. необъявленным в качестве 
           параметров функции) можно сделать через коллекцию arguments[]        
               https://learn.javascript.ru/arguments-pseudoarray  
        */
            function proc3() { 
               // Вывод первых двух аргументов в консоль. Может не работать в движке Gecko:
               console.log("Функцией получены аргументы: " + arguments[0] + " и " + arguments[1]);  
            }

    // ВЫЗОВ ФУНКЦИИ С АРГУМЕНТАМИ //
        
        /* В круглых скобках через запятую можно передать в функцию произвольное количество 
           аргументов независимо от того, обрабатываются ли они каким-то образом в функции или нет.
           В аргументах могут передаваться не только значения, но и функции, объекты, массивы и т.д.
        */
        proc3(17, "ещё один аргумент", true);
    
    // ПАРАМЕТРЫ ФУНКЦИИ //
    
        /* При объявлении функции можно в скобках перечислить имена параметров, которым будут 
           присвоенны значения аргументов. Эти параметры станут локальными переменными функции.
        */
        function proc4(width, height) { // Указано два параметра с именами width и height
            console.log("Функцией proc4 получены данные: ", {width, height});
            /* Значение параметра можно по неосторожности заменить на другое. Компилятор ошибки не 
               покажет (кроме старых браузеров). Эту особенность можно использовать для проставления 
               значений по умолчанию (если аргумент не передан в функцию) */
            var width = 65, height = 75; 
            console.log("Данные внезапно изменились: ", {width, height});
        }
        proc4(35,45); //   {35,45}  ,  {65,75}
         
    // ПАРАМЕТРЫ ПО УМОЛЧАНИЮ //
        /* Если передать аргументов меньше, чем объявлено параметров, то для непереданных аргументов 
           параметры останутся со значениями undefined */
        proc4(8);  // {8, undefined}
        
        // Если значение undefined не устраивает, то можно организовать в теле функции его замену:
        function proc5(value) { 
            if (value === undefined) value = 0; // Очевидный способ
            value = value || 0; // Компактный но менее наглядный способ (особенность оператора `||`)
            console.log("Функцией proc5 получено значение ", {value});
        }
        proc5(4);      // 4
        proc5();       // 0, а не `undefined`
        proc5(null);   // 0, т.к. оператор `||`  срабатывает не только на `undefined`
    }
    f01(); 
    console.groupEnd("Процедурное программирование");
  }
  
  {/* НИСХОДЯЩИЕ ФУНКЦИОНАЛЬНЫЕ АРГУМЕНТЫ И КОЛБЭКИ

        Как было сказано ранее, если функция передаётся в другую функцию в виде аргумента, то такой
        аргумент называется функциональным (т.е. фунаргом). Чаще всего такая переданная через 
        аргумент функция в процессе дальнейшей работы программы вызывается изнутри той функции, 
        которая её приняла. В таком случае она называется колбэком (callback) - т.е. функцией
        "обратного вызова".
        Иными словами, если функция a(b) содержит инструкцию b(); - значит b является колбэком.
            https://learn.javascript.ru/function-expressions#funktsii-kolbeki
   */
    console.groupCollapsed("Колбэки");

    var f02 = function() { 
    
    // ИМЯ ФУНКЦИИ КАК ПЕРЕМЕННАЯ //
            
        /* Имя функции без скобок является переменной, хранящей ссылку на объект-функцию.
           Если сделать приведение этого объекта с строке, то возратится код функции
                https://learn.javascript.ru/function-declaration-expression  
        */
        function proc1() {}
        str1 = "" + proc1; 
        console.log("Код функции: " + str1);
        
        var proc2 = proc1; // Функцию можно продублировать в новую переменную
        // При этом в свойстве `name` функционального объекта хранится изначальное имя функции:
        console.log("Изначальное имя функции:", proc2.name); // "proc1"
        
    // ПРОЦЕДУРЫ ВЫСШЕГО ПОРЯДКА //
        // Это процедуры,  которые принимают в качестве аргумента другую процедуру или функцию
        
        function showHi() { console.log("Здравствуйте!"); }
        function showBye() { console.log("До свидания!"); }
        
        function callFunc(f) { f(); } // Простейшая ПВП, просто вызывающая переданную процедуру
        callFunc(showHi);  // "Здравствуйте!"
        callFunc(showBye); // "До свидания!"
        
    // ОТСУТСТВИЕ ПРОБЛЕМЫ НИСХОДЯЩЕГО ФУНАРГА // 
    
        // Допустим у нас есть переменная в глобальной области и процедура, выводящая её значение: 
        str1 = "Глобальное значение";
        var fVar = function showStr1() { console.log(str1); }
       
        // Простая процедура с функциональным аргументом:
        function faDemo(funArg) { 
            var str1 = "Локальное значение 1"; // Создадим одноимённую локальную переменную
            funArg(); // При динамическом окружении вывелось бы "Локальное значение 1"
        }
        
        /* Как можно видеть при передаче функции как функционального аргумента, она выдаёт результат
           такой же, как при прямом вызове - благодаря лексическому, а не динамическому окружению.
        */
        fVar();         // "Глобальное значение"
        faDemo(fVar);   // "Глобальное значение"
    
    // Поведение при вызове дочерних подпрограмм в другом динамическом контексте //
    // (т.е. не из родительской процедуры)                                       //
        
        /* Создадим также другую функцию, которая подменяет ссылку в переменной fVar так, чтобы она
           указывала на подфункцию в другой динамической области видимости:
        */
        var setfVar = function() { 
            var str1 = "Локальное значение 2"; 
            var subFunc = function() { console.log(str1); } // Подфункция в локальной области 
            fVar = subFunc; 
            subFunc = null; /* Для усложнения ситуации удалим подфункцию. Тем не менее это ни на что 
                               не повлияет т.к. обнуляется только ссылка subFunc, а сами данные 
                               функционального объекта остаются в памяти и на них ссылается fVar
                               (благодаря автоматическому управлению памяти в JavaScript) */
        }
        
        setfVar();      // Помещаем в fVar ссылку на подфункцию subFunc, а саму подфункцию удаляем
        setfVar = null; /* Очищаем ссылку на родительскую функцию. Тем не менее это не удаляет её 
                           объект, т.к. на него продолжает ссылаться поле [[outer]] функции fVar */
        
        // В итоге функция fVar продолжает запускаться, но уже с телом функции subFunc:
        fVar();         // "Локальное значение 2"
        faDemo(fVar);   // "Локальное значение 2"
        
        /* Как видим, при повторных вызовах функции она теперь использует значение переменной Str1,
           заданное в локальном окружении подфункции (а не как раньше в глобальном). В то время
           как при динамическом связывании и отсутствии автоматического управления памятью у нас бы 
           появилась ошибка из-за удаления подфункции и её динамического окружения.
           В данном случае была рассмотрена проблема нисходящего фунарга (т.е. когда функция 
           передаётся внутрь другой функции). Обратный вариант (когда функция возвращает свою
           подфункцию) рассмотрен в блоке "Функциональное программирование".
        */
        
    // АСИНХРОННЫЙ ВЫЗОВ ПРОЦЕДУРЫ ИЛИ ФУНКЦИИ ПО ТАЙМЕРУ //
    
        function f1(){}; // Эту функцию будем передавать в качестве аргумента в функцию setTimeout()
        ...
        clearTimeout(timer1); // Отмена таймера, (сработает если функция ещё не успела запуститься)
        setTimeout(f1, 100, 3, 4); // Аргументы 3,4,5,.. будут переданы в вызываемую функцию
        
    // ВЫЗОВ ПРОЦЕДУРЫ ИЛИ ФУНКЦИИ ЧЕРЕЗ ОПРЕДЕЛЁННЫЕ ИНТЕРВАЛЫ //
        ...
        // Задание пауз между вызовами (с помощью рекурсии)
            var timer3 = setTimeout(function f2() {  // Тут функция подставлена сразу как аргумент
   ...
   */
    ...
    // ФУНКЦИИ ВЫСШЕГО ПОРЯДКА
        // - это функция, принимающая функциональный аргумент и возвращающая другую функцию
        
        function applyFunc3(f,a,b,c){ // Объявление функции принимающей на вход другую функцию
            var x = f(a,b); // В этом примере к параметрам a,b,c попарно применяется функция f
            return f(x,c);
        }
        function sum(a,b) { // Функция сложения, используемая для передачи в качестве паратметра
            return a + b;
        }
    ...
    // АНОНИМНЫЕ ФУНКЦИИ //
       ...
       /* Это функции которые объявляются прямо там где используются без сохранения в именованную 
          переменную. Поскольку эти функции выполняются сразу при их определении, то они имеют 
          название Immediately Invoked Function Expression (IIFE). Т.е. "немедленно вызываемое 
       ...
       */  
        ...
        // К числам применяется функция умножения, созданная на лету:
           ...
           /* Поскольку в аргументе функции может быть только выражение, а не инструкция, то 
              компилятор понимает, что это не простая декларация, а функциональное выражение */
        
        /* В тех частях кода, где можно размещать инструкции, анонимную функцию можно создать, 
           обернув её объявление в скобки: 
        */
            (function (s){
    ...
    // ОБЛАСТЬ ВИДИМОСТИ ФУНКЦИЙ И ФУНКЦИОНАЛЬНЫХ ВЫРАЖЕНИЙ //
        ...
        /* Для разграничения области видимости используются именованные функциональные выражения 
           (NFE - Named Function Expression), т.е. когда в переменную записывается именная функция:
             https://learn.javascript.ru/named-function-expression  
        */
        var f1 = function myFunc(){  /* Внутри тела функции f1 - глобальная переменная , а myFunc - 
                             локальная, которую нельзя изменить снаружи (что важно для рекурсии) */
    ...
    // ПРОСТОЕ ИСПОЛЬЗОВАНИЕ ВЛОЖЕННЫХ ФУНКЦИЙ //
        ...
        var decoString = function(str) {
            function deco(c,n) { // Подфункция декорирования строки по краям другой строкой
                function mul(ord){  // Функция повторения декорируемой строки
                   ...
                   if (ord==0) s = s.split("").reverse().join("");  // Разворот строки для ord == 0
                   return s;
                }
    ...
    // ВОЗВРАТ ВЛОЖЕННОЙ ФУНКЦИИ С СОЗДАНИЕМ ЗАМЫКАНИЯ //
      ...
      */
        var makeCounter = function() {  // Функция подсчитывающая количество своих запусков
            var currentCount = 0; // Счетчик - локальная переменная родительской функции
                       
            /* Возвращаемая вложенная функция создаёт свой объект [[LexicalEnvironment]] в котором 
               поле [[outer]]  ссылается на [[LE]] функции makeCounter, в котором хранится 
               переменная currentCount. 
            */  
            return function() {          // Вложенная функция при вызове увеличит
                return ++currentCount;   // значение счетчика и возвратит его 
            };                          
        }
        
        /* Чтобы создалось замыкание, надо возвращаемую вложенную функцию присвоить в переменную. 
           Тогда родительская функция хоть и закончила работу, и освободила стек исполнения, но 
           созданная переменная ссылается на её дочерний функциональный объект, который в свою 
           очередь ссылается на [[LE]] родительской функции, а значит этот объект [[LE]] не может 
           быть удалён из памяти, пока переменную не обнулить или не изменить. А значит все 
           изменения хранящихся в этом объекте переменных будут храниться и доступны при вызове 
           этой вложенной функции.
        */
        var c1 = makeCounter(); // [[scope]] функции c1 ссылается на [[LE]] функции makeCounter    
        
        /* При вызове с1 создаёт свой [[LE]], в котором нет переменной currentCount, но есть ссылка 
           [[outer]], по которой можно получить значение currentCount родительского [[LE]]: 
        */
        c1(); // Функция увеличила currentCount в [[LE]] родителя
        c1(); c1(); i = c1(); // Каждый вызов увеличивает счётчик и возвращает результат
        ...
        console.log("Функции c1() и с2() запускались " + i + " и " + j + " раза.");  // 4 и 3
    } 
    f03();
    console.groupEnd("Функциональное программирование");
  } console.groupEnd("Подпрограммы (функции)");
}
...
{// ОРГАНИЗАЦИЯ ЛОГИКИ ПРОГРАММЫ //
  ...
  console.groupCollapsed("Рекурсия");
    ...
    // Рекурсия со вспомогательной вложенной функцией для хранения промежуточного значения
  ...
  {/* ПЕРЕХВАТ ОШИБОК ВЫПОЛНЕНИЯ 
    ...
    // Простая обработка ошибок, выявляемых интерпретатором
        try { // Конструкция try…catch позволяет предотвратить падение скрипта в случае ошибки.
            ...
            console.log( 'Это сообщение не будет отображено.'); // После ошибки код не выполняется
        ...
        finally { console.log( 'Выполнение блока закончилось.' ); } 
...
{// СОСТАВНЫЕ СТРУКТУРЫ ДАННЫХ //
  ...
  {/* СТАТИЧЕСКИЕ ЧИСЛОВЫЕ ТИПИЗИРОВАННЫЕ МАССИВЫ 
    ...
    if (0) { // эти методы не работают в старых браузерах
  ...
  console.groupCollapsed("Динамические нетипизированные массивы");
    ...
    // Создание и инициализация массива
        ...
        // Если в такой записи указать один числовой аргумент, то он задаст длину массива
    ...
    // Чтение и запись элементов массива
        ...
        arr1[5] = "А"; // Если записать данные в ячейку с индексом выходящим за границы 
                       // массива, то размер массива автоматически увеличится до указанного индекса
    ...
    // Перебор элементов массива
        for (i=0; i < arr1.length; i++) { arr1[i]=0; }
    ...
    // Приведение строки к массиву (парсинг)
        ...
        arr1 = str1.split("");  // Если разделитель пустая строка, то разбиение будет посимвольное:
        ...
        console.log('Результат split("",4): ' + arr1); // 1,2,3,4
    ...
    /* Поиск по массиву. Метод indexOf работает аналогично методу indexOf для строк
           Можно указать вторым аргументом индекс, с которого осуществлять поиск. 
    ...
    /* Применение функций к элементам массива 
        ...
        arr1b = arr1.map(function(item, index) {
                return [item, arr1a[index]];
            });
        console.log("Результат zip-операции с массивами: элемент arr1b[8] == " + arr1b[8]); 
  ...
  {/* БЕСКЛАССОВЫЕ ОБЪЕКТЫ И ИХ СВОЙСТВА 
  ...
  console.groupCollapsed("Создание объектов и хранение в них данных");
    ...
    // Создание пустого объекта
        var obj1 = new Object();
        var obj2 = {};
        var obj3 = Object.create(null); // Создание максимально простого объекта (без прототипов):
    ...
    // Работа со свойствами как с ассоциативным массивом
        ...
        // У всех обычных объектов есть системное свойство __proto__
        if ("__proto__" in obj1) console.log("Это обычный объект"); // true
        // Поэтому во избежание конфликтов ассоциатные массивы лучше создавать через Object.create: 
        obj2 = Object.create(null);
        if (! ("__proto__" in obj2)) console.log("Это простейший объект"); // true
    
    // Переменное имя переменной
        /* Поскольку все переменные хранятся как свойства текущего объекта, то можно воспользоваться
           этим для обращения к переменной, имя которой хранится в строке */
    
        var varName1 = "numb2"; // В переменной varName1 хранится имя переменной numb2
        this[varName1] = 456;   // Создана или изменена переменная с именем "numb2"
        str1 = "В переменной " + varName1 + " хранится значение " + this[varName1];        
        console.log(str1);    
    ...
    // Создание объекта с групповым перечислением свойств:
        ...
        var vcard2 = {  // Запись в несколько строк более наглядна
            ...
            city: "Москва",  // У последнего свойства можно оставить запятую - это не вызовет ошибку
        };
        
    // Свойство совпадающее по имени с переменной
        str1="Строка 1";
        str2="Строка 2";
        // Если в имя свойства нужно записать значение из одноимённой переменной, то
        obj3 = { str1:str1, str2:str2 }; // вместо этой инструкции 
        obj3 = { str1, str2 };           // можно написать такую, более короткую.
        console.log(obj3);            
    ...
    // Создание полной независимой копии объекта (глубокое клонирование)
        ...
        console.log("Изменение vcard2.phones.home=123 затрагивает копию ссылки, но не клон:" +
                    "\n    vcard2.phones.home : "     + vcard2.phones.home     +  // == 123
                    "\n    vcard2link.phones.home : " + vcard2link.phones.home +  // == 123
                    "\n    vcard2copy.phones.home : " + vcard2copy.phones.home);  // == 789
...
{// ОБЪЕКТНО-ОРИЕНТИРОВАННОЕ ПРОГРАММИРОВАНИЕ //
  ...
  {/* МЕТОДЫ ОБЪЕКТОВ 
      (Методом называется функция, являющаяся свойством объекта)
...
console.groupEnd("Преобразование объектов к примитивным типам");
  ...
  console.groupCollapsed("Сериализация объектов (в формат JSON)");
    ...
    /* Использование прокси-функции для пользовательской сериализации (во втором аргументе метода).
  ...
  {/* КОНСТРУКТОРЫ ОБЪЕКТОВ 
      (Конструкторы - это, функции создающие новые функции-объекты)
   ...
   */
    ...
    // Конструктором становится любая функция, вызванная с ключевым словом new
        ...
        /* При создании объекта конструктором у объект наследует свойство constructor, 
           которое хранит ссылку на эту самую функцию-конструктор */
        console.log("Контруктор объекта f1: " + f1.constructor); // "function() {}"
        // Это свойство удобно использовать для создания "братьев" объекта
        var f2 = new f1.constructor(); // то же что и  f2 = new emptyFunc()
...
console.groupEnd("Конструкторы объектов");
   ...
   */
    console.groupCollapsed("Прототипное наследование");
    
    /* В ES5 наследование реализовано путём указания для объекта ссылки на другой объект, 
       который для него будет прототипом. */ 
        var parent1 = {}; 
        var child1 = {};
        /* Переопределение спец.свойства __proto__ создаёт в объекте спец.свойство [[Prototype]], 
           которое хранит ссылку на другой объект как прототип */
        child1.__proto__ = parent1;  // Теперь у child1 прототип parent1
        // Object.setPrototypeOf(child1, parent1); // Современный способ переопределения прототипа
        /* Крайне важно прототип указывать при создании объекта, т.к. изменение прототипа для 
           действующего объекта приводит к кардинальному падению производительности скрипта */
        var child1 = Object.create(parent1); // Современный способ создания объекта с прототипом
        var grandChild1 = { __proto__: child1 }; // Можно выстраивать иерархию потомков
    
    /* Указание прототипа в конструкторе 
        https://learn.javascript.ru/function-prototype  */
        
        /* Если объект создаётся конструктором, то указать прототип для объекта можно создав в
           функции-конструкторе свойство prototype, ссылающееся на объект-прототип. 
           При создании объекта таким конструктором в __proto__ будет записан указанный прототип. 
           После того как объект создан, свойство prototype никакой роли в наследовании не играет.*/
            var GrandChild2 = function() { this.opt = 2; }
            GrandChild2.prototype = child1;  
            var obj1 = new GrandChild2();  // У obj1 прототип child1
            GrandChild2.prototype = parent1;
            var obj2 = new GrandChild2();  // У obj2 прототип parent1
            // Проверка:
            child1.name = "child1";   console.log("Прототип obj1: " + obj1.__proto__.name);   
            parent1.name = "parent1"; console.log("Прототип obj2: " + obj2.__proto__.name);   

    /* Прототип по умолчанию */
            var Child2 = function() {}    
            /* У любой функции по умолчанию создаётся свойство prototype, в которое записывается 
               новый объект с один единственным свойством constructor, указывающим на эту функцию. 
               Таким образом новая функция изначально хранит внутри себя ссылку на саму себя. */
            console.log(Child2.prototype.constructor == Child2); // true
            /* Это означает, что если использовать эту функцию в качестве конструктора, 
               то созданные объекты будут иметь своим прототипом такой объект: 
                 { constructor: <ссылка на функцию-конструктор> }     */
            var obj1 = new Child2();
            console.log(obj1.__proto__); // {constructor: function Child2 }
            console.log(obj1.__proto__.constructor); // Child2() {}
    
    /* Наследование не переопределённых свойств и методов */
        parent1.opt = 1;
        parent1.showOpt = function() { return this.opt; };
        /* Не найденные в объекте свойства и методы берутся из ближайшего по иерархии прототипа - 
          в данном случае из parent1 : */
        console.log("Обращение к свойству прототипа: " + child1.opt );    // 1
        console.log("Выполнение метода прототипа: " + grandChild1.showOpt() ); // 1 
    
    /* Переопределение свойств и методов в потомках */
        grandChild1.opt = 2; // Свойство переопределено в объекте, а не в прототипе
        // В наследованных методах ссылка this ссылается на текущий объект, а не на прототип:
        console.log("Пуск метода прототипа на изменённом объекте: "+grandChild1.showOpt()); // 2
        child1.showOpt = function() { return "Это метод child1"; }; // Метод переопределён
        console.log("Пуск переопределённого метода: " + child1.showOpt()); // "Это метод child1"
    
    /* Обращение к свойствам и методам прототипа */
        str1 = grandChild1.__proto__.opt; // 1
        console.log("Для прототипа объекта grandChild1 свойство opt == " + str1); // 1
        // str1 = Object.getPrototypeOf(child1).showOpt(); // Современный способ ссылки на прототип
        console.log("Пуск метода прототипа вместо метода объекта == " + str1); // 1
    
    /* Проверка, является ли свойство собственным или унаследованным */
        if (grandChild1.hasOwnProperty('opt')) console.log("Объект grandChild1 имеет свойство opt");
          
console.groupEnd("Прототипное наследование");
}    

  {/* ДЕСКРИПТОРЫ СВОЙСТВ 
      (Дескриптор свойства - объект хранящий значение, параметры, геттеры и сеттеры свойства)
...
console.groupEnd("Дескрипторы свойств");
  ...
  {/* ШАБЛОНЫ ПРИМЕНЕНИЯ МЕТОДОВ 
   */  
    console.groupCollapsed("Шаблоны применения методов");
    ...
    // Фиксирование аргументов, передаваемых в функцию:
    ...
    console.groupEnd("Шаблоны применения методов");
