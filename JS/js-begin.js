...
console.groupEnd("Приведение простых типов при операциях");        
  ...
  console.groupCollapsed("Процедурное программирование");
    ...
    // Вызов функций без аргументов
        ...
        proc2(); // В режиме Function Declaration вызов функции можно делать до объявления функции 
        
    /*  Использование аргументов, переданных в функцию. Доступ к необъявленным аргументам 
		в теле функции - через коллекцию arguments[]        
	...
	/* Вызов функций с аргументами и демонстрация области видимости переменных. */
		...
		str2 = "Глобальное значение"; // Внешняя переменая
        proc3(35,45); // Функция изменила глобальную переменную str1 (плохое поведение)
  ...
  console.groupCollapsed("Функциональное программирование");
    ...
    // Имя функции как переменная. Функциональные выражения
        ...
        var func1c = function(){   // Объявление функции в режиме Function Expression
            console.log('Выполняется процедура proc1с');
        }
        func1c(); /* Если функция объявлена в режиме Function Expression вызов функции можно 
                     делать только после объявления функции */
        func1c = null; // Переменную содержащую функциональное выражение можно обнулить
    ...
    /* Анонимные функции - это функции которые объявляются прямо там где используются, 
       ...
       определении, то они имеют название Immediately Invoked Function Expression (IIFE).       */
        ...
        //Того же эффекта можно достичь, если обернуть функцию в скобки
        (function (s){
...
console.groupCollapsed("Составные структуры данных");
  ...
  console.groupCollapsed("Работа с датой и временем");
    ...
    // Средства для бенчмаркинга
        ...
        console.time('Вычисление 100 тыс. раз факториалов от 0 до 30 функцией 1');
            for (i=1; i<100000; i++) for (j=1; j<30; j++) k = factorial(j);
        // Функция console.timeEnd() выводит значение таймера с указанной меткой
        console.timeEnd('Вычисление 100 тыс. раз факториалов от 0 до 30 функцией 1');
        console.time('Вычисление 100 тыс. раз факториалов от 0 до 30 функцией 2');
            for (i=1; i<100000; i++) for (j=1; j<30; j++) k = factorial2(j);
        console.timeEnd('Вычисление 100 тыс. раз факториалов от 0 до 30 функцией 2');
         
        /* Если в браузере не задействовано округление таймера с целью защиты от атак, то функция 
           performance.now() возвращает зачение вплоть до микросекунд */ 
  ...
  console.groupCollapsed("Функции как объекты");
    ...
    // Возврат функции-объекта, свойствами которого являются функции-методы
        function makeTimer(){ // Создаём простой секундомер для замеров времени
            var startTime = performance.now(); // Переменная будет хранить время начала отсчета
...
console.groupCollapsed("Преобразование объектов к примитивным типам");
    ...
    // Логическое преобразование (производится там где ожидается получение boolean) 
		...
		// Специальный метод Array.isArray() позволяет прверить, является ли аргумент массивом:
...
console.groupEnd("Дескрипторы свойств");
  ...
  console.groupCollapsed("Статические и фабричные методы консрукторов");
	...
	/* Статические свойства и методы - те, которые не привязаны к экземплярам объектов
		...
		function PixelStat(){ // Конструктор пикселей, который считает сколько пикселей создал
	...
	// Независимая функция работы с объектом (объявленная вне конструктора или объекта)
		function distance(){  // Сторонний метод вычисления расстояния от угла экрана до пикселя
	...
	// Каррирование функций (создание производных функций с меньшим количеством аргументов)
		function mul(a, b) { return a * b; };  // Функция умножения от двух аргументов
	...
	/* Функция-обёртка или декоратор (функция которая принимает на вход  функцию 
	   и возвращает её, снабдив новыми возможностями)  
		...
		function numCheck(f) { // Функция проверки, являются ли аргументы функции числами
			return function() {
