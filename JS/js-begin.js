...
{/* КОМАНДЫ ВЗАИМОДЕЙСТВИЯ С ПОЛЬЗОВАТЕЛЕМ */
  ...
  { /* Вывод текстовых строк  
    ...
    */
        ...
        // Если заключить строки или переменные в фигурные строки, то это всё выведется как объект:
...
console.groupEnd("Приведение простых типов при операциях");        
  ...
  {/* ПРОЦЕДУРНОЕ ПРОГРАММИРОВАНИЕ 
    ...
    */
    ...
    /* Функция - это блок программы, который принимает на вход аргументы 
      ...
      В таком режиме функции можно помещать в блоки инструкций `{}`. В связи с этими особенностями 
      весь дальнейший код обёрнут в блок функционального выражения:
    ...
    // ОБЛАСТЬ ВИДИМОСТИ ПЕРЕМЕННЫХ //
        ...
        /*  В ES5 реализована только функциональная видимость переменных (в отличие от ES6, где есть
            ...
            случае, если родительский объект перестал существовать до завершения работы функции. 
                ...
                1. Глубокое связывание - функция выполняется в контексте своего описания в коде
                   программы (используется лексическое окружение).
                2. Теневое связывание - функция выполняется в контексте своего вызова (используется 
                   динамическое окружение).
                3. Специальное связывание - создаётся копия контекста выполнения, каким он был на 
                   момент запуска функции, и уже из него функция берёт все необходимые значения. 
    ...
    // ПАРАМЕТРЫ ФУНКЦИИ //
        ...
        */
        ...
        proc4("Только один аргумент"); /* Если передать аргументов меньше задуманного, то для 
                            непереданных аргументов параметры останутся со значениями undefined */
    }
    f01(); 
    console.groupEnd("Процедурное программирование");
  }
  
  {/* КОЛБЭКИ (НИСХОДЯЩИЕ ФУНКЦИОНАЛЬНЫЕ АРГУМЕНТЫ)
   ...
   */
    ...
    // ИМЯ ФУНКЦИИ КАК ПЕРЕМЕННАЯ //
        ...
        /* Имя фукнции без скобок является переменной, хранящей ссылку на объект-функцию.
    ...
    // ПЕРЕДАЧА ФУНКЦИИ В ПАРАМЕТР ДРУГОЙ ФУНКЦИИ //
    
        function showHi() { console.log("Здравствуйте!"); }
        function showBye() { console.log("До свидания!"); }
        
        /* Функции, которые принимают в качестве аргумента другую функцию называются функциями 
           высшего порядка (ФВП или higher-order functions) 
        */
        function callFunc(f) { f(); } // Простейшая ФВП, просто вызывающая переданную функцию
        callFunc(showHi);  // "Здравствуйте!"
        callFunc(showBye); // "До свидания!"
        
    // ОТСУТСТВИЕ ПРОБЛЕМЫ НИСХОДЯЩЕГО ФУНАРГА // 
    
        // Допустим у нас есть переменная в глобальной области и функция выводящая её значение: 
        str1 = "Глобальное значение";
        var fVar = function showStr1() { console.log(str1); }
       
        // Простая функция с функциональным аргументом:
        ...
        /* Как можно видеть при передаче функции как функционального аргумента, она выдаёт результат
           такой же, как при прямом вызове - благодаря лексическому, а не динамеческому окружению.
        */
        ...
        /* Создадим также другую функцию, которая подменяет ссылку в переменной fVar так, чтобы она
           указывала на подфункцию в другой области видимости:
        */
        ...
        // В итоге функция fVar продолжает запускаться, но немного по другому:
        ...
        /* Как видим, при повторных вызовах функции она теперь использует значение переменной Str1,
           ...
           как при динамическом связывании и отсутствии автоматического управлению памяти у нас бы 
   ...
   */
    ...
    // РАБОТА С ПОЛУЧЕНЫМ ФУНКЦИОНАЛЬНЫМ АРГУМЕНТОМ //
