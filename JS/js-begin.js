...
"use strict"; /* Эта директива, если стоит первой, переводит интерпретатор в режим повсеместно 
                 ...
                 написанными в старых стандартах.  https://learn.javascript.ru/strict-mode */
  ...
  {/* РАБОТА С ПЕРЕМЕННЫМИ   
    ...
    */
    ...
    // Объявление и инициализация переменных
        ...
        var numb1 = 546; /* При повторном объявлении вместе с инициализацией также произойдёт смена 
                            значения (т.е. слово var будет проигнорировано) */
    ...
    // Всплытие var-объявлений
        /* Т.к. объявить переменную можно в любом месте скрипта, то все var-объявления движок JS
           воспринимает так, будто бы они объявлены в самом начале скрипта, но без инициализации. 
           Такое явление называют всплытием объявления переменных. При этом сами инициализации 
           выполняются в том месте скрипта, где они написаны: */
           
        console.log(str3); // Undefined , т.к. до этого в переменную ничего не записывалось.
        str3 = "Значение 1";
        console.log(str3); // "Значение 1"
        var str3 = "Значение 2"; /* Это объявление переменной уже после её реального использования 
          будет как бы условно заменено на `str3 = "Значение 2";`, а в начале скрипта (опять же как
          бы условно) будет помещено объявление `var str3;` */
        console.log(str3); // "Значение 2"
    ...
    */
    ...
    // ОБЛАСТЬ ВИДИМОСТИ ПЕРЕМЕННЫХ //
        ...
        /*  В ES5 реализована только функциональная видимость переменных (в отличие от ES6, где есть
            ...
            правильно обращаться со внешними переменными важно понимать, какой вариант доступа к ним
            реализован под капотом JS.
            ...
            название такого внешнего окружения функции объектами называется "лексическим окружением" 
            (или "статическим окружением"). В ряде других языков программирования используется 
            иной тип окружения - динамическое, при котором родителем для функции выступает функция 
            или подпрограмма, из которой она была вызвана.
            ...
            в разных других функциях и неоднозначен либо потерян контекст её выполнения.
            
            В JavaScript замыкание реализовано следующим образом. В момент запуска функции у неё 
            создаётся ещё одно свойство - специальный объект с именем [[LexicalEnvironment]] 
            (т.е. "лексическое окружение"). В этом объекте сохраняются все переменные, объявленные 
            внутри функции или переданные в неё, а также создаётся запись [[envRec]] ("запись 
            окружения"), в которую обычно просто копируется свойство [[Scope]] самой функции.
            
            В итоге получается, что запущенный на выполнение экземпляр функции содержит внутри себя 
            все свои внутренние данные, а также ссылку на её родительский объект, в котором ищутся
            переменные, не найденные в собственном [[LexicalEnvironment]]. Если отсутсвующая 
            переменная не найдена и в родительском [[LexicalEnvironment]], то происходит переход по
            родительской [[envRec]] на родитель родителя и т.д. по цепочке родителей, пока не будет
            либо найдена сама переменная, либо достигнут объект window, у которого [[envRec]]==null.
    ...
    // ЛОКАЛЬНЫЕ ПЕРЕМЕННЫЕ ФУНКЦИИ //
        ...
        // Демонтстрация того, что область видимости лексическая, а не динамическая
    ...
    // ИМЯ ФУНКЦИИ КАК ПЕРЕМЕННАЯ //
        ...
        */
        ...
        var func1c = proc1b; // Функцию можно продублировать в новую переменную
   ...
   */
    ...
    // ВОЗВРАТ ВЛОЖЕННОЙ ФУНКЦИИ С СОЗДАНИЕМ ЗАМЫКАНИЯ //
      ...
      /* Каждая функция при запуске создаёт спец.объект LexicalEnvironment, в который копируются 
         аргументы, функции и переменные функции. Это позволяет в каждой новой созданной функции 
         хранить свой набор переменных. При создании функции присваивается специальное 
