...
console.groupEnd("Дескрипторы свойств");
  ...
  {/* Статические и фабричные методы конструкторов 
  ...
  console.groupCollapsed("Статические и фабричные методы консрукторов");
	...
	// Статические свойства и методы - те, которые не привязаны к экземплярам порождаемых объектов
		function PixelStat(){ // Конструктор пикселей, который считает сколько пикселей создал
			if (PixelStat.count == undefined) PixelStat.count = 0; // Статическое свойства .count, 
	...
	// Вызов функции для посторонних объектов (в другом контексте)
		function distance(){  // Сторонний метод вычисления расстояния от угла экрана до пикселя
			return Math.sqrt( Math.pow(this.x,2) + Math.pow(this.y,2) );
		};	// Этот метод можно вызвать разными вариантами:
		
		// Можно "испортить" объект, записав функцию в свойство и вызвать как метод:
		pixel1.dist = distance; 
		k = pixel1.dist(); // == 500
		console.log("Расстояние от угла экрана до pixel1: " + k );
		
		// Но лучше применить метод к объекту встроенным методом call:
		distance.call(pixel1); // == 500
		...
		// Методом bind можно создать новую функцию, постоянно привязанную к указанному контексту
		...
		// Вместе с закреплением this можно закрепить первые N аргументов, передаваемых в функцию
		...
		// Каррирование функций (вызов функции с меньшим количеством аргументов)
