...
{/* ХРАНЕНИЕ И ОБРАБОТКА ДАННЫХ В ОПЕРАТИВНОЙ ПАМЯТИ. */
  ...
  {/* НОВЫЙ ТИП ДАННЫХ SYMBOL
          https://habr.com/ru/companies/ruvds/articles/444340/
          https://learn.javascript.ru/symbol
    */
    console.groupCollapsed("Новый тип данных Symbol");
    
    /* Новый тип данных Symbol является примитивным (значимым) типом, но при этом ведёт себя как 
       объект и хранится в объекте. Как и объекты, каждый новый создаваемый символ уникален и не 
       равен никакому другому символу. Можно предположить, что создаётся некий уникальный набор 
       байт, который не доступен из программного кода - он и является значением symbol-переменной.
       Особенности символьного типа особо полезны при использовании при создании свойств объектов.
    */
    
    let sym1 = Symbol(); // Символ создаётся вызовом конструктора Symbol()
    console.log('Создан символ:', {sym1}); // Внутреннее значение символа не отображается
    
    // Для символов не работает автоматическое приведение к строке - нужно использовать toString():
    let str = 'Создан символ: ' + sym1.toString(); // И так тоже значение не отображается
    console.log({str});

    let sym2 = Symbol("Описание символа");  // Можно передать в конструктор строку с описанием
    // Это описание никак не влияет на значение символа и используется обычно только для отладки
    let sym3 = Symbol("Описание символа");  // Новый символ с тем же описанием не равен sym2:
    console.log('Обычные символы с идентичным описанием не равны:', sym2 != sym3); // true
    
    str = 'Описание: ' + sym2.description;  // Описание хранится в свойстве `description`
    console.log({str});

    // Глобальные символы //
    
        /* С помощью метода Symbol.for() можно создать глобальный символ, который будет единственным
           возможным уникальным символом для заданного описания. Повторные вызовы метода вместо 
           создания нового символа будут возвращать созданное при первом запуске значение: 
        */
        let gsym1 = Symbol.for('Строка1'); // Создан новый глобальный символ
        let gsym2 = Symbol.for('Строка1'); // Возвращено значение существующего глобального символа
        console.log('Глобальные символы с идентичным описанием равны:', gsym1 === gsym2); // true
        
        // Метод Symbol.keyFor() возвращает description глобальных символов:
        console.log('Для глобальных символов оба способа получения описания работают одинаково :',
                     Symbol.keyFor(gsym1) === gsym1.description);  // true
        
        /* А вот для локальных символов (в силу отсутсвия однозначного соответсвия символа описанию)
           метод возвращает undefined. Это можно использовать для проверки глобальный ли символ: */
        
        if (Symbol.keyFor(sym1) === undefined) console.log('Символ sym1 - локальный.'); // true
        if (!Symbol.keyFor(sym1)) console.log('Символ sym2 - локальный.'); // Более краткий вариант
        
    console.groupEnd("Новый тип данных Symbol");
...
https://learn.javascript.ru/function-expressions#function-expression-v-sravnenii-s-function-declaration
  ...
  {/* СПРЕД-СИНТАКСИС И ОСТАТОЧНЫЕ ПАРАМЕТРЫ
      https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Operators/Spread_syntax
      https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Functions/rest_parameters
    */
    console.groupCollapsed("Спред-синтаксис и остаточные параметры");
    
    /* В ES6 появилась возможность напрямую передать массив в функцию таким образом, чтобы элементы 
       массива стали отдельными параметрами внутри функции. Для этого используется т.н. оператор 
       размазывания (spread), состоящий из троеточия и добавляющийся перед именем массива.
    */
    
    // Имеем простую функцию с несколькими аргументами:
    function showArgs(a,b,c,d,e,f,g,h) { 
        console.log('Получены аргументы:',a,b,c,d,e,f,g,h); 
    }
    
    let array1 = [1,2,3,4];
    console.log('Результаты этих четырёх вызовов одинаковы:');
        showArgs(1,2,3,4);             // Передача обычных аргументов в функцию
        showArgs.apply(null, array1);  // Передача массива по-старинке, в стандарте ES5
        showArgs(...array1);           // Передача массива в спред-синтаксисе
        showArgs(...[1,2,3,4]);        // Передача литерала массива в спред-синтаксисе
        
    
    // Можно произвольно чередовать размазанные массивы с обычными аргументами
        showArgs(7, 4, ...array1, 5, 3, ...[9,2], 6);
        
    /* Такое же троеточие, как в спред-операторе, можно использовать для обратной операции собирания 
       нескольких поступивших в функцию аргументов в один параметр функции, являющийся массивом. 
       Такой синтаксис называется rest-синтаксисом (т.е. остаточным), а элементы получаемого массива 
       называются остаточными параметрами (rest-параметрами).

       Этот массив остаточных праметров должен стоять последним и единственным в череде обычных 
       параметров (ведь заранее неизвестно, сколько аргументов может быть передано в функцию):
    */
    
    function showArgs2(a,b, ...array2) { 
        console.log('Все параметры функции:',{a},{b},array2);
    }
    
    // Идентичная по поведению функция в стандарте ES5 получается более громоздкой.
       
    function showArgs3(a,b) {
        var array3 = []; 
        for (var i=2, j=0; i < arguments.length; i++) array3[j++] = arguments[i];
        console.log('Все параметры функции:',{a},{b},array3);
    }
    
    console.log('Тестируем остаточные параметры:');
...
{/* ФУНКЦИОНАЛЬНОЕ ПРОГРАММИРОВАНИЕ */
  ...
  {/* СОЗДАНИЕ СТРЕЛОЧНЫХ ФУНКЦИЙ 
    ...
    */
    ...
    /* Если у стрелочной функции нет аргументов или их больше одного, то необходимы круглые скобки,
       внутри которых и размещаются нужные аргументы. Но если аргумент всего один, то круглые
       скобки можно опустить. Две функции ниже полностью идентичны:
    */   
      let sqr1 = function(x) { return x*x; };
      let sqr2 = x => x*x;
      console.log("Квадрат числа равен" + sqr2(5)); // 25

    // Удобно создавать и анонимные стрелочные функции, вызывая их на лету:
