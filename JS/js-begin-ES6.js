...
{/* ХРАНЕНИЕ И ОБРАБОТКА ДАННЫХ В ОПЕРАТИВНОЙ ПАМЯТИ. */
  ...
  console.groupCollapsed("Строки в обратных кавычках (шаблонные литералы)");
    ...
    /* Также этот новый формат позволяет помещать внутрь строки произвольный JS-код, результат 
       выполнения которого будет выведен в виде текста в указанное место строки.
       Для этого внутри обратнокавычной строки используется синтаксическая конструкция ${} */
       str = `Дважды два равно ${2*2}`; // Выражение внутри фигурных скобок вычисляется
       console.log(str);
    
       // На практике это удобно применять для подставления в строку значений переменных
        let x=3, y=4;
