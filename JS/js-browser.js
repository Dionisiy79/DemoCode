...
{ /* ОБРАБОТКА СОБЫТИЙ */
	wr("<h2> Обработка событий на примере события клика мыши </h2>");
	
	// Самый примитивный способ - прописать тело анонимной функции в атрибут onclick прямо в HTML
		wr('<input type="button" id="button1" value="Старт" onclick="alert(\'Сработал код №1\')"');
	
	// Но лучше управлять управлять атрибутом через JS
		var onclickDemo = function () {
			...
			/* Если у фукнции обработчика объявить аргумент, 
			   то он станет объектом, хранящим параметры события */
			button1.onclick = function(eventObj) { 
				alert("Сработало событие с типом " + eventObj.type +
				                  " для элемента " + eventObj.currentTarget + 
					        ". Координаты клика: " + eventObj.clientX + " / " + 
					                                 eventObj.clientY);
			}		
		}
		// Чтобы элемент button1 успел появиться в DOM, ставим небольшую задержку
		setTimeout(onclickDemo, 2000);
	...
	var eventListenerDemo = function() { 
		button1.addEventListener("click", alert4); // Добавление обработчика alert3 на событие click
		button1.removeEventListener("click", alert4); // Удаление функции alert3 из обработчика 
	...
	setTimeout(eventListenerDemo, 2000);
