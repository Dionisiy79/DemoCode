...
<div> <h2> ПРОЦЕДУРНОЕ ПРОГРАММИРОВАНИЕ </h2>
  ...
  <div>    <h3>  Работа с аргументами </h3>
	<?php   	  
	// Доступ к необъявленным аргументам в теле функции - через функцию func_get_arg()
        function proc4() { 
	...
	// Передача в функцию не значения, а ссылки на переменную: 
		function proc8(&$a) {      // Для этого при объявлении аргумента нужно указать символ "&"
			$a .= " + дополнение"; // Это присвоение меняет глобальную переменную по ссылке
		}
		$str2 = "Изначальное значение";
		proc8($str2);  // Изменяем переменную str2
