...
<div>
<h1>ОСНОВЫ РАБОТЫ НА ЯЗЫКЕ ПРОГРАММИРОВАНИЯ PHP</h1>
...
<div> <h2> ОБЩИЕ СИНТАКСИЧЕСКИЕ ПРАВИЛА </h2>
	...
	<p> Если выражение содержит ошибку и некорректный результат вычисления, то интерпретатор PHP 
	    выведет ошибку или предупреждение прямо в HTML-документ. Предупреждение можно 
        принудительно скрыть, если прямо перед выражением поставить символ "@"</p>
		<?php  
		    (1 + $var);  // Это выражение выведет предупреждение WARNING: Undefined variable $var
			@(1 + $var); // Символ "@" отключает вывод предупреждения
		?>
...
<div> <h2> ХРАНЕНИЕ И ОБРАБОТКА ДАННЫХ В ОПЕРАТИВНОЙ ПАМЯТИ </h2>
  ...
  <div>    <h3> РАБОТА С ПЕРЕМЕННЫМИ  </h3>
	<div>     <h4> Создание удаление переменных </h4>
	<?php   /* https://www.php.net/manual/ru/language.variables.basics.php 
		...
		время исполнения позволяет создавать переменные с именем, хранящимся в другой переменной.
	...
	<div>     <h4> Работа с константами (неизменяемыми переменными) </h4>
	...
	<div>     <h4> Использование переменных </h3>
	<?php   
  ...
  <div>    <h3> ОПЕРАЦИИ </h3>
	...
	<?php 
        ...
        $numb1 = 31 % 8;           // == 7     Остаток от деления целых чисел
	...
	<?php   
		...
		### ЯВНОЕ ЧИСЛЕННОЕ ПРЕОБРАЗОВАНИЕ ###
			...
			###	ПРИВЕДЕНИЕ К ВЕЩЕСТВЕННОМУ ЧИСЛУ ###
				...
				// Конструкции (float) и (double) приводят к double
		...
		### ПОЛУЧЕНИЕ КОДА СИМВОЛА ###
			$numb1 = ord("4"); // Функция ord возвращает первый байт строки в виде числа
				echo "<br>Код однобайтного символа '4': $numb1"; //  52
			$numb1 = mb_ord("Б"); // Числовой код первого символа для любых кодировок
				echo "<br>Код unicode-символа 'Б': $numb1"; // 1041
	?>
	</div>     	
  ...
  <div>    <h3> МАТЕМАТИЧЕСКИЕ ФУНКЦИИ И КОНСТАНТЫ </h3>
	<?php 	// https://www.php.net/manual/ru/math.constants.php
		...
		$numb1 = rand(1,100); // Генерация псевдослучайного целого в выбранном диаппазоне 
		$numb1 = rand();      // Генерация псевдослучайного целого в диаппазоне от 0 до getrandmax()
...
<div> <h2> ХРАНЕНИЕ И ОБРАБОТКА ДАННЫХ В ФАЙЛАХ И БД </h2>
 
  <div>    <h3> РАБОТА С ФАЙЛАМИ </h3>
	<?php  // https://www.php.net/manual/ru/ref.filesystem.php
		
	### СОЗДАНИЕ ФАЙЛА И ЗАПИСЬ В НЕГО ###
		$f1 = fopen("./file1.txt", "w"); /* Открытие файла для записи. В переменной f1 - дескриптор
		    файла. Если файл не существует, то он будет создан. Существующий файл - перезаписан. */
		fwrite($f1, "Строка1.\n");
		fwrite($f1, "Строка2."); // Пока файл открыт, запись будет осуществляться в конец файла.
		fclose($f1); // Закрытие файла
	
	### ПРОВЕРКА СУЩЕСТВОВАНИЯ ФАЙЛА ###
		if (file_exists("./file1.txt")) echo "file1.txt является файлом или каталогом.";
		if (is_file("./file1.txt")) echo "<br>file1.txt является обычным файлом.";

	### ЧТЕНИЕ ФАЙЛА ###
		$f1 = fopen("file1.txt", "r"); // Открытие файла для чтения.
		$char1 = fgetc($f1); // Чтение однобайтного символа начиная с текущей позиции
		/* Чтобы прочитать двухбайтный кириллический символ, нужно проверить считанный байт, не 
		   является ли он первым байтом многобайтного символа: */
		$n=ord($char1); 
		if (($n == 208) || ($n == 209) || ($n == 129)) {
		    $char1 .= fgetc($f1); // Считываем второй байт символа
		}
		$str0 = fread($f1,4); // Прочитать 4 байта начиная с текущей позиции и записать в строку
		$str1 = fgets($f1);   // Прочитать строку начиная с текущей позиции
		$str2 = fgets($f1);   // Считываем следующую строку
			echo "<br>Считан символ '$char1' и строки '$str0', '$str1', '$str2'";
		if (feof($f1))	echo "<br>Достигнут конец файла"; // Проверка достижения конца файла
		fclose($f1);		

	### ДОПИСЬ В ФАЙЛ ###
		$f1 = fopen("./file1.txt", "a"); // Открытие файла для дописи в конец
		fwrite($f1, "\nСтрока3."); // Эта строка запишется в конец файла
		fclose($f1);		

	### ЧТЕНИЕ ВСЕГО ФАЙЛА В СТРОКОВУЮ ПЕРЕМЕННУЮ ###
		$text1 = file_get_contents('./file1.txt');
			echo "<br>Содержимое файла: $text1";
	
	### СОЗДАНИЕ ФАЙЛА ИЗ СТРОКОВОЙ ПЕРЕМЕННОЙ ###
		file_put_contents('./file2.txt', $text1); // Полная перезапись файла
		file_put_contents('./file2.txt', $text1, FILE_APPEND); // Допись в конец
	
	### МАНИПУЛЯЦИИ С ФАЙЛАМИ ###
		copy('./file2.txt','./file3.txt'); // Копирование файла
		rename('./file3.txt','./file4.txt'); // Переименование или перемещение файла 
		unlink('./file1.txt'); // Удаление файла
		unlink('./file2.txt'); unlink('./file4.txt');
	
	### МАНИПУЛЯЦИИ С КАТАЛОГАМИ ###
		mkdir("myDir");  // Создание каталога
		if (is_dir("myDir")) echo "<br>Каталог 'myDir' существует."; // Проверка существования
		$str = getcwd(); // Полный путь к текущему каталогу
			echo "<br>Текущий каталог: $str"; 
		chdir("myDir");  // Смена каталога 
		chdir("..");     // Возврат на уровень выше
		rename("myDir","myDir2");  // Переименование каталога 
		rmdir("myDir2");  // Удаление каталога
	?>
  </div>   
</div>
...
<div> <h2> ПРОЦЕДУРНОЕ ПРОГРАММИРОВАНИЕ </h2>
  ...
  <div>    <h3>  ОБЛАСТЬ ВИДИМОСТИ ПЕРЕМЕННЫХ СПОСОБЫ ИНИЦИАЛИЗАЦИИ </h3>
	<?php   	  
        ...
        echo "<br> Первый вызов: "; proc3(); //	"s2: Начальное значение" 	
        echo "<br> Второй вызов: "; proc3(); // "s2: Начальное значение + дополнение" 	
...
<div> <h2> ФУНКЦИОНАЛЬНОЕ ПРОГРАММИРОВАНИЕ </h2>
  ...
  <div>    <h3> ВОЗВРАЩЕНИЕ ЗНАЧЕНИЙ И ССЫЛОК ИЗ ФУНКЦИИ </h3>
	...
	### ВЫЗОВ ФУНКЦИИ, ИМЯ КОТОРОЙ ХРАНИТСЯ В СТРОКОВОЙ ПЕРЕМЕННОЙ ###
		...
		// Если после имени переменной указать скобки () то будет вызвана функция с именем в строке:
  ...
  <div>    <h3> ЗАМЫКАНИЯ И ДРУГИЕ ОСОБЕННОСТИ РАБОТЫ С ФУНКЦИЯМИ </h3>
    ...
    ### ОБЛАСТЬ ВИДИМОСТИ ФУНКЦИЙ И ВЛОЖЕННЫХ ФУНКЦИЙ ###
        { function localFunc1() {
	...
	### ВИДИМОСТЬ ВНЕШНИХ ПЕРЕМЕННЫХ У ВЛОЖЕННЫХ ФУНКЦИЙ РАЗНЫХ ВИДОВ ###
		/* Использовать во вложенной функции переменную из родительской функции можно только если
		   наследовать её конструкцией use во вложенную анонимную функцию. Простое наследование 
		   передаёт значение на момент объявления (а не вызова) функции. Для возможности доступа 
		   к актуальному значению, а также его изменению, нужно подключать переменную по ссылке. */
        $str1 = "глобальная переменная";
        function lexEnvTest() {
            $str1 = "начальное значение локальной переменной функции lexEnvTest";
            $func1 = function() use ($str1) { return $str1; };   // Передача значения переменной
            $func2 = function() use (&$str1) { return $str1; };  // Передача ссылки на переменную
			// Доступ к глобальной переменной можно получить и без анонимной функции:
            function func3() { global $str1; return $str1; };  
			$str1 = "изменённое значение локальной переменной";
            echo "<br>Вложенная функция func1 видит, что str1 == ".$func1(); // начальное значение
            echo "<br>Вложенная функция func2 видит, что str1 == ".$func2(); // именённое значение
            echo "<br>Вложенная функция func3 видит, что str1 == ".func3(); // глобальная переменная
        }
        lexEnvTest();

	### ПРОСТОЕ ИСПОЛЬЗОВАНИЕ ВЛОЖЕННЫХ ФУНКЦИЙ ###
        function decoString($str) {
            $deco = function($c,$n) use ($str){ // Подфункция декорирования строки другой строкой
                $mul = function ($ord) use ($c,$n){  // Функция повторения декорируемой строки
                   $s = ""; for ($i=1; $i<=$n; $i++) $s .= $c;  
                   if ($ord == 0) $s = strrev($s);  // Разворот строки для ord == 0
                   return $s;
                };
        ...
        echo "<br>Результат работы вложенных функций: $str1";

    ### ВОЗВРАТ ВЛОЖЕННОЙ ФУНКЦИИ С СОЗДАНИЕМ ЗАМЫКАНИЯ ###
		/* Каждая функция при запуске сохраняет в замыкании свои аргументы, функции и переменные. 
		   Это позволяет в каждой новой созданной функции хранить свой набор переменных */
        function makeCounter() {  // Функция подсчитывающая количество своих запусков
            $currentCount = 1;    // Счетчик - локальная переменная главной функции
            return function() use (&$currentCount) { // Возвращается вложенная функция хранящая 
                // используемое ею лексическое окружение (в данном случае - переменную currentCount)
                return $currentCount++;  // Вложенная функция при вызове возвратит 
            };                           // значение счетчика и увеличит его
        }
        $c1 = makeCounter();     /* Запуск функции возвращающей функцию создаёт и хранит 
                                       лексическое окружение для c1() */
        $c1(); $c1(); $c1(); $i = $c1(); // Каждый вызов увеличивает счётчик и возвращает результат
        $c2 = makeCounter();     // Новый созданный счётчик будет независим от первого т.к.
        $c2(); $c2(); $j = $c2();       // у него собственное лексическое окружение
        echo "<br>Функции c1() и с2() запускались $i и $j раза."; // 4 и 3
...
<div> <h2> ОРГАНИЗАЦИЯ ЛОГИКИ ПРОГРАММЫ </h2>
  ...
  <div>    <h3> ВЕТВЛЕНИЯ В ЛОГИКЕ ПРОГРАММЫ </h3>
    ...
    ### ОПЕРАЦИЯ ОБЪЕДИНЕНИЯ С NULL ###
        /* https://www.php.net/manual/ru/migration70.new-features.php#migration70.new-features.null-coalesce-op 
           Оператор ?? возвращает первый операнд, если он существует и не равен null, 
           иначе возвращает второй операнд. Доступен в версиях PHP 7.0 и выше:
        */
        
        $str1 =  $condivar_x ?? 'не найдено'; // = 'не найдено', если $condivar_x не существует 
        
        /* Т.о. операция позволяет избежать ошибки `Undefined variable`, являясь т.н. синтаксическим 
           сахаром, заменяющим такой более громоздкий код: */
        $str1 = isset($condivar_x) ? $condivar_x : 'не найдено';
        
        /* В PHP 7.4 и выше доступен комбинированный оператор ??= позволяющий при присвоении 
           значений не дублировать имя переменной, присваивая ей саму себя: */
           
        $condivar_x ??= 7; // Если переменная не существует, то записать в неё значение 7
        // По факту операция делает ровно то же самое что:
        
            $condivar_x = $condivar_x ?? 7;
	?>
  </div>   
  <div>    <h3> БЕЗУСЛОВНЫЕ УПРАВЛЯЮЩИЕ КОММАНДЫ </h3>
	...
	### ПОЛНОЕ ПРЕКРАЩЕНИЕ ВЫПОЛНЕНИЯ СКРИПТА ###
		if(0) { exit;     // Простое завершение работы
				exit("Случилась ошибка."); // Завершение с выводом текстового сообщения
				exit(17); // Завершение с числовым кодом ошибки 17
		} 
	?>
  </div>   
  <div>    <h3> ЦИКЛЫ </h3>
  ...
  <div>    <h3> РЕКУРСИЯ </h3>
   ...
   ### РЕКУРСИЯ ЧЕРЕЗ МНОЖЕСТВЕННЫЙ ВЫБОР ##
    ...
    ### РЕКУРСИЯ СО ВСПОМОГАТЕЛЬНОЙ ВЛОЖЕННОЙ ФУНКЦИЕЙ  ###
		...
		function prime2($n) { // Функция проверки числа на простоту (возвращает true или false)
            $pr_ = function($n,$m) use (&$pr_) { // Локальная вспомогательная функция. Аргумент m
												 // хранит число, до которого произведена проверка
                return $m*$m > $n ? true  : // Все кандидаты на делители проверены, возвращаем true      
                      ($n%$m == 0 ? false : // Делитель найден, возвращаем false
                      $pr_($n,$m+1));  // Рекурсивная проверка следующего кандидата на делителя
            };
            return $n > 0 ? $pr_($n,2) : NaN; // Запускаем вспомогательную функцию
        }  									  // проверки кандидатов на делители начиная с 2
        $str1='';
...
<div> <h2> СОСТАВНЫЕ СТРУКТУРЫ ДАННЫХ </h2>
  ...
  <div>    <h3> ДИНАМИЧЕСКИЕ НЕТИПИЗИРОВАННЫЕ МАССИВЫ </h3>
	...
	### ЧТЕНИЕ И ЗАПИСЬ ИНДЕКСИРОВАННЫХ ЭЛЕМЕНТОВ ###
        ...
        $arr1[5] = "А"; /* Если записать данные в ячейку с индексом, выходящим за "границы" массива,
	...
	### Перебор элементов массива с вызовом функции для каждого (альтернатива циклу foreach) ###
		...
		// Функция array_walk () применяет к массиву функцию из второго аргумента:
...
<div> <h2> ОБЪЕКТНО-ОРИЕНТИРОВАННОЕ ПРОГРАММИРОВАНИЕ </h2>
  ...
  <div>    <h3> СОЗДАНИЕ КЛАССОВ И ОБЪЕКТОВ </h3>
	...
	<?php	
		...
		### СОЗДАНИЕ ПУСТОГО ОБЪЕКТА (НА ОСНОВЕ КЛАССА) ###
			...
			### Анонимный класс (для создания единичных объектов на лету) ###
			$obj3 = new class {};
			echo "<br>Объекты: "; print_r($obj1); print_r($obj2); print_r($obj3);

		### ОБЪЯВЛЕНИЕ КОНСТАНТ КЛАССА ###  
			class ClassCon { // Константы доступны в версиях PHP 7.1+
				public    const CON1 = 1;   
  ...
  <div>    <h3> ИСПОЛЬЗОВАНИЕ ОБЪЕКТОВ </h3>
	...
	<?php	
		...
		### ДОСТУП К ПУБЛИЧНЫМ КОНСТАНТАМ ###
			/* Константа хранится в классе - общая для всех объектов класса. Для доступа к ней нужно
			   использовать оператор разрешения области видимости ::  
			   */
			$n = ClassCon::CON1; 
				echo "<br>Костанта CON1 == $n";
			$className = "ClassCon"; 
			$n = $className::CON1; // Альтернативный способ через переменную, хранящую имя класса
			$n = ClassCon::class; // Встроенная константа class хранит полное имя класса
				echo "<br>Костанта class == $n";

		### ДОСТУП К ПУБЛИЧНЫМ СТАТИЧЕСКИМ СВОЙСТВАМ ###
			$s = ClassStat::$stat1;
				echo "<br>Статическое свойство stat1 == $s";
			ClassStat::$stat1 += 1; // Статическое свойство можно менять
			$className = "ClassStat";			
			$s = $className::$stat1; // Доступ через переменную, хранящую имя класса
				echo "<br>Изменённое статическое свойство stat1 == $s";
		...
		### ДОСТУП К ПУБЛИЧНЫМ СВОЙСТВАМ ###
			...
			$n = $obj2->$propName; // Альтернативный способ - через переменную хранящую имя свойства
				echo "<br>Новое значение свойства numb1 == $n";
			// В новых версиях PHP динамическое добавление свойств и методов запрещено
		...
		/* Создавать новые методы динамически нельзя. В старых версиях PHP можно записать в свойство 
           функцию, но в последних версиях это тоже запретили для повышения надёжности кода */
  ...
  <div>    <h3> СЕРИАЛИЗАЦИЯ ОБЪЕКТОВ </h3>
	<?php	
		...
		### Пользовательская сериализация - с преобразованием значений  ###
			...
			$vcard1 = new VcardSerial();
			$vcard1->name = "Иван"; $vcard1->age = 28;
		...
		### Пустой конструктор и деструктор ###
			class EmptyConstruct {
				function __construct() {
					echo "Сработал конструктор класса EmptyConstruct.";
				}
				function __destruct() {
					echo "<br>Сработал деструктор класса EmptyConstruct.";
				}
			}
			$obj1 = new EmptyConstruct(); // Тут сработает контруктор
			$obj1 = null; // На объект больше нет ссылок - он уничтожится и сработает деструктор.

		### Изменение свойств создаваемых объектов в конструкторе ###
			class RandPixel { 
				public $x, $y;
				function __construct() {
					$this->x = rand(); $this->y = rand();
			}}
			$pix1 = new RandPixel(); // Создан пиксель со случайными координатами
			echo "<br>Конструктором создан объект - случайный пиксель: "; print_r($pix1);
  ...
  <div>    <h3> НАСЛЕДОВАНИЕ </h3>
	<?php	
		...
		### ЗАПРЕТ НАСЛЕДОВАНИЯ ###
			final class FClass {} // Объявление класса как final запрещает создание дочерних классов
		...
		### ЗАПРЕТ ПЕРЕОПРЕДЕЛЕНИЯ МЕТОДОВ ###
			class ParentClassFixed {
				final public function met1() {} // Метод помеченный как final переопределять нельзя
			}
		...
		### ПРОВЕРКА ПРИНАДЛЕЖНОСТИ ОБЪЕКТА КЛАССУ ###
			...
			$b = ($child1 instanceof ChildClass1)  && // child1 - экземпляр класса ChildClass1
			     ($child1 instanceof ParentClass1) && // и класса ParentClass1 (как наследник)
				 is_a($child1,"ParentClass1")      && // Альтернативная функция-аналог 
				 is_subclass_of($child1,"ParentClass1");// child1 - потомок ParentClass1
			echo "<br>Результат проверки принадлежности классам: $b"; // true

		 ### ВЫЯСНЕНИЕ КЛАССА ОБЪЕКТА ###
			echo "Имя собственного класса у child1: ".get_class($child1);         // "ChildClass1"
			echo "Имя родительского класса у child1: ".get_parent_class($child1); // "ParentClass1"
			// Вместо объекта можно указать имя класса в виде строки:
			echo "Родитель для ChildClass1: ".get_parent_class("ChildClass1");  // "ParentClass1"
			// Имена всех родительских классов в виде массива:
			echo "Список предков объекта child1: "; print_r(class_parents($child1)); 
			// Таким образом можно узнавать класс текущего объекта из методов:
			class ParentClass2 {
				public function getClassName() { return get_class($this); }
				public function getParentClassName() { return get_parent_class($this); }
				public function getParentsClNames() { return class_parents($this); }
				// Указатель $this не работает в статических методах. Для них есть такая функция:
				static public function getCalledClassName() { return get_called_class(); }
			}			
			class СhildClass4 extends ParentClass2 {};
			$obj1 = new СhildClass4();
			echo "<br>Определение класса для obj1 из вызываемых методов:".
				 "<br>Свой класс: ".$obj1->getClassName().                           // СhildClass4
			     ". Родительский класс: ".$obj1->getParentClassName().               // ParentClass2
			     ". Определение класса в стат.методе: ".$obj1->getCalledClassName(); // СhildClass4
	?>
  </div>
  <div>    <h3> АБСТРАКТНЫЕ КЛАССЫ И МЕТОДЫ </h3>
	<?php	
		/* Абстрактный класс - это класс, для которого нельзя создавать объекты. Его можно 
		   использовать только для создания дочерних классов. */

		### ОБЪЯВЛЕНИЕ ПУСТОГО АБСТРАКТНОГО КЛАССА ### 
			abstract class EmptyAClass {}
			
		### ОБЪЯВЛЕНИЕ АБСТРАКТНЫХ МЕТОДОВ ###
			/* Абстрактный метод - это декларация названия метода и его аргументов без тела метода.
			   Определение тела метода (т.е. реализация) обязано присутствовать в дочерних классах.
			   Абстрактные методы можно определять только в абстрактных классах. */
			abstract class AClass1 {
				abstract protected function met1();
				abstract protected function met2($arg); // Можно указать обязательный аргумент
				// В абстрактном классе можно объявлять неабстрактные методы:
				public function echoMet1() { echo "Метод met1 сообщает: ".$this->met1(); }
			}

		### РЕАЛИЗАЦИЯ АБСТРАКТНЫХ МЕТОДОВ В ДОЧЕРНЕМ КЛАССЕ ###
			class userClass1 extends AClass1 {
				protected function met1() { return "Это userClass1!"; } 
				// Можно расширять область видимости, но не сужать (protected изменено на public)
				public function met2($arg,$arg2=1) { // Можно добавлять дополнительные аргументы,
				    // но для них нужно обязательно указывать значение по умолчанию.
					return "<br>Получены аргументы $arg, $arg2";
				}
			}
			$obj1 = new userClass1();
			$obj1->echoMet1(); 
			echo $obj1->met2("строка");
	?>
  </div>
  <div>    <h3> ИНТЕРФЕЙСЫ </h3>
	<?php	
		/* Интерфейс - это именованный набор деклараций публичных методов. При наличии интерфейсов 
		   классы можно объявлять, как классы реализующие те или иные интерфейсы. При этом такие 
		   классы обязаны содержать в себе реализацию задекларированных в интерфейсе методов. 
		   Итерфейс описывает ЧТО умеет делать класс, но не описывает КАК он это делает. */

		### ОБЪЯВЛЕНИЕ ИНТЕРФЕЙСА ### 
			interface interface1 {
				public function met1($arg1, $arg2); // Метод должен быть публичным
				const CON1 = "Текст"; // Можно объявить константу интерфейса
			}
		
		### ИСПОЛЬЗОВАНИЕ ИНТЕРФЕЙСОВ ###
			class interClass1 implements interface1 { // Класс, реализующий интерфейс
				public function met1($arg1, $arg2) {return $arg1+$arg2;} // Складываем числа
			} // В классе нельзя объявлять константы, одноимённые интерфейсным константам
			// В другом классе можно объявить другую реализацию интерфейса
			class interClass2 implements interface1 {  
				public function met1($arg1, $arg2) {return $arg1.$arg2;} // Склеиваем строки
			}
			$obj1 = new interClass1;
			$obj2 = new interClass2;
			echo "<br>Работа интерфейса obj1: ".$obj1->met1(1,2); // 3
			echo "<br>Работа интерфейса obj2: ".$obj2->met1(1,2); // 12
			echo "<br>Константа интерфейса: ".interface1::CON1;   // Доступ к константе
			
		### НАСЛЕДОВАНИЕ ИНТЕРФЕЙСОВ ###
			interface childInterface1 extends interface1 { // интерфейс расширяет родительский
				public function met2($arg1);
			} 
			// Допустимо множественное наследование интерфейсов:
			interface childInterface2 extends interface1, JsonSerializable {
				public function met2($arg1);
			} /* В классах, реализующих дочерний интерфейс, должны быть прописаны реализации всех 
			     методов как дочернего, так и родительских интерфейсов */
		
		### МНОЖЕСТВЕННАЯ ИМПЛЕМЕНТАЦИЯ ИНТЕРФЕЙСОВ ###
			// Один класс может реализовывать много интерфейсов:
			class interClass3 implements childInterface1,childInterface2 { 
				// Обязательная реализация всех методов родительских интерфейсов
				public function met1($arg1, $arg2) {return $arg1.$arg2;} 
				public function jsonSerialize():mixed { return []; } 
				/* Метод met2 объявлен в обоих интерфейсах. Это допустимо только если 
				   его декларация в этих интерфейсах совпадает. */
				public function met2($arg1) { echo  $arg1.$arg2; }
			}

		### ПРОВЕРКА НАЛИЧИЯ ИНТЕРФЕЙСА У ОБЪЕКТА ИЛИ КЛАССА ###
			// Во всех нижепреведенных сравнениях результат равен true:
			$b = ($obj1 instanceof interface1)    && // obj1 реализует интерфейс interface1
				 is_a($obj1,"interface1")         && // Альтернативная функция-аналог 
				 is_subclass_of($obj1,"interface1"); // obj1 числится потомком interface1
			echo "<br>Результат проверки наличия интерфейсов у obj1: $b"; // true
			
			// Список интерфейсов объекта или класса в виде массива:
			echo "<br>Все интерфейсы у obj1: "; print_r(class_implements($obj1));
			echo "<br>Интерфейсы класса interClass3: "; print_r(class_implements("interClass3"));
	?>
  </div>
  <div>    <h3> ТРЕЙТЫ </h3>
	<?php	
		/* Трейт (характерная особенность) - это абстрактный тип, аналог интерфейса, который 
		   в отличие от интерфейса содержит не только декларацию, но и реализацию публичных методов. 
		   При подключении трейта к разным классам можно использовать эти публичные методы в этих 
		   классах. Это удобно для повторного использования кода (чтобы не дублировать одинаковые 
		   методы в разных классах). 
		   https://www.php.net/manual/ru/language.oop5.traits.php  */

		### ОБЪЯВЛЕНИЕ ТРЕЙТА ### 
			trait trait1 {
				public function trMet1() { // Метод должен быть публичным
					return "Сработал метод trMet1 трейта 1"; 
				} 
				public static function trMet2() { // Метод может быть статическим
					return "Сработал метод trMet2 трейта 1"; 
				} 
				public $var1 = 1; // Можно объявить свойство трейта
			}  // Статические свойства недопустимы, но можно использовать стат.переменные в методах
			trait trait1abs {
				abstract public function trMet3(); // Метод может быть абстрактным
			} // Тогда в классах, использующих трейт, должна быть прописана реализация метода.
			
		### ИСПОЛЬЗОВАНИЕ ТРЕЙТОВ ###
			class traitClass1 { 
				use trait1; // Использование трейта в классе
				public function getVar1() { return $this->var1; } // Доступ к свойству трейта
			} 
			// В другом классе можно использовать тот же трейт 
			class traitClass2 { use trait1; public function trMet3() {} }
			$obj1 = new traitClass1;
			$obj2 = new traitClass2;
				echo "<br>Вызов метода трейта в obj1: ".$obj1->trMet1(); 
			$obj1->var1++; // Свойство из трейта можно менять. В других объектах оно не меняется
				echo "<br>Значение публичного свойства трейта в obj1: ".$obj1->getVar1(); // 2
				echo "<br>Работа обычного метода трейта в obj2: ".$obj2->trMet1(); 
				echo "<br>Значение публичного свойства трейта в obj2: ".$obj2->var1; // 1
			
		### ПЕРЕОПРЕДЕЛЕНИЕ МЕТОДОВ ###
			class trParentClass {
				public function met1() { return 'Это метод родительского класса.'; }
			}
			trait trait2 { 
				public function met1() { return 'Это метод трейта.'; } // Одноимённый метод в трейте
				// К унаследованному методу всё ещё можно обратиться через parent::
				public function callParentMet1() { return parent::met1(); } 
			}
			class trChildClass1 extends trParentClass {  // Унаследован метод met1()
				use trait2; // Метод трейта met1() приоритетнее унаследованного метода met1()
			}
			class trChildClass2 extends trParentClass { 
				use trait2; // Одноимённый метод класса приоритетнее метод трейта:
				public function met1() { return 'Это метод дочернего класса.'; }
			}
			$obj1 = new trChildClass1;
			$obj2 = new trChildClass2;
				echo "<br>Вызов родительского метода: ".$obj1->callParentMet1();  
				echo "<br>Вызов метода трейта в классе 1: ".$obj1->met1();  
				echo "<br>Вызов метода класса в классе 2: ".$obj2->met1();  
		
		### ИСПОЛЬЗОВАНИЕ НЕСКОЛЬКИХ ТРЕЙТОВ ###
			class multiTrClass1 {
				use trait1, trait2; // Можно перечислить несколько трейтов через запятую
			}
			/* Если в используемых трейтах есть одноимённые методы, то нужно явно указать какой из
			   них использовать вместо другого одноимённого */
			trait trait3 { // Трейт в котором существует одноимённый метод для trait2
				public function met1() { return 'Это метод другого трейта.'; }  
			}
			class multiTrClass2 {
				use trait2, trait3 {
					trait3::met1 insteadof trait2; // Указание использовать метод из trait3
					trait2::met1 as altMet1; // Псевдоним для замещённого метода
				}
			}
			$obj1 = new multiTrClass2;
			echo "<br>Вызов метода по обычному имени: ".$obj1->met1(); // "Это метод другого трейта"
			echo "<br>Вызов метода по псевдониму: ".$obj1->altMet1();  // "Это метод трейта"
		
		### ИЗМЕНЕНИЕ ОБЛАСТИ ВИДИМОСТИ МЕТОДОВ ТРЕЙТА В КЛАССЕ ###
			class trPrivClass1 {
				use trait1 { trMet1 as private; } // Простое изменение области видимости метода
			}
			class trPrivClass2 {
				use trait1 { trMet1 as private met1; } // Создание псевдонима с другой видимостью
			}
		
		### ИСПОЛЬЗОВАНИЕ ТРЕЙТА В ТРЕЙТЕ ###
			trait multiTrait { // Этот трейт объединяет методы из используемых им трейтов
				use trait1, trait2; 
			}
			class multiTrClass3 {
				use multiTrait; // В данном случае заменяет конструкцию:  use trait1, trait2;
			}
