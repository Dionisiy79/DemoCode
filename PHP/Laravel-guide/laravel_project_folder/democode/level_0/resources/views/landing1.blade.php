...
<div>
    ...
    <h3> Передача переменных в шаблон </h3>
        <p> Таким же способом можно выводить переменные, переданные в шаблон вместе со view-объектом из колбэков `view()`. Для случая, если какие-то из переменных не были переданы, то можно избежать появления ошибки `Undefined variable` можно путём использования условного оператора `??` :
    ...
    <h3> Подключение дочерних шаблонов </h3>
        ...
        <p> Шаблонизатор Blade имеет целых три инструмента для вставки внутрь шаблона переиспользуемого кода:
            <ol> <li>Включаемые подшаблоны </li>
                 <li>Наследуемые шаблоны </li>
                 <li>Компоненты </li>
            </ol>
        
        <p> Для статических страниц подойдёт самый простой вариант - включаемые подшаблоны. Допустим, мы решили на всех страницах сайта отображать главное меню. Для этих целей создадим файл `menu1.blade.php` с простейшей навигацией по трём страницам сайта.
        ...
        <p> Дополнительно при подключении можно передать переменные в виде ассоциативного массива во втором аргументе директивы: `@@include('имя шаблона',['имя'=>'значение', .. ])`: 
        </p>    
