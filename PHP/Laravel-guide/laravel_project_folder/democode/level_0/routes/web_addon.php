...
#------------------------------------#
    ...
    Route::view('/landingpage_1', 'landing1')->name('land1');
...
#----------------------------------------------------#
    ...
    /* В blade-шаблонах можно использовать переменные и фрагменты PHP-кода. Примеры такого включения смотрите в файле шаблона landing1. 
    ...
    */
    ...
    Route::get('/landingpage_1a', function () {
...
#----------------------------#
    ...
    Создадим в папке `resources\views\` эту подпапку и в ней пару шаблонов:
...
#-----------------------------------#
    ...
    Чтобы не усложнять задачу, для статических страниц применим самый простой вариант - включаемые подшаблоны.
    Допустим, мы решили на всех страницах сайта отображать главное меню. Для этих целей создадим файл `menu1.blade.php` с навигацией по трём страницам сайта (см. папку `resources\views\`). Ссылки в меню сделаем по-умному - на именованные маршруты.

    Теперь, когда подшаблон подготовлен, его можно включить в шаблоны секций и в шаблон лендинга, добавив в нужном месте каждого из этих шаблонов директиву `@include('menu1')` (см. файл `landing1.blade.php`*/