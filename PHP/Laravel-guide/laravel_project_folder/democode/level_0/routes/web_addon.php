...
#-----------------------------------#
    ...
    Чтобы не усложнять задачу, для статических страниц применим самый простой вариант - дочерние шаблоны.
    Допустим, мы решили на всех страницах сайта отображать главное меню. Для этих целей создадим файл `menu1.blade.php` с навигацией по трём страницам сайта (см. папку `resources\views\`).

    Теперь, кода дочерний шаблон подготовлен, его можно включить в шаблоны секций и в шаблон лендинга, добавив в нужном месте каждого из этих шаблонов директиву `@include('menu1')` (см. файл `landing1.blade.php`*/