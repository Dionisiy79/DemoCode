...
Если помимо URL-параметров нужны GET-параметры, и их хочется получить не просто через хелпер `request()`, а через внедрение зависимости в виде Request-объекта, то в аргментах функции эта и другие зависимости должна стоять перед URL-параметрами маршрута:
