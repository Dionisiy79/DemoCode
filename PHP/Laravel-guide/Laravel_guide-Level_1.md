...
Попытка аналогичным образом использовать два параметра приводит к тому, что второй параметр оказывается с пустым значением, а предназначенная для него строка оказывается добавленной к строке первого параметра:
...
Получение данных от пользователя может приходить не только через GET-запросы, но и через POST, PUT и пр. Использовавшийся нами ранее метод `get()` не среагирует на POST-запрос. Для работы с разными видами запросов в Laravel предусмотрены следующие инструменты.
...
#### Получение POST-данных из HTML-форм

При получении данных из POST-запроса вместо Request-метода `query()` используется Request-метод `input()`. Этот метод обрабатывает данные как из POST-запроса, так и адресную строку с GET-параметрами. Но чтобы не вносить путаницу удобнее разделять ответсвенность и GET-параметры обрабатывать методом `query()`, а всё остальное - методом `input()`. В качестве компактной альтернативы можно получать значение поля формы через одноимённое динамическое свойство Request-объекта, которое фреймворк автоматически создаёт.

```php
    Route::post('/post_search1', function (Request $request) {
        $s = 'Ищем текст "' . $request->input('s_string') .
             '" на языке '  . $request->input('lang','ru').
             '<br>Текст в динамическом свойстве: ' . $request->s_string;

        $s .= '<br> Все параметры запроса в виде массива: ';
        foreach ( $request->input() as $key=>$value )
            $s .= "$key => $value (тип: " . gettype($value) . ") | ";

        return $s;
    })->name('level1_post_search1');
```
При необходимости с помощью Request-методов `only()` и `expect()` можно получить в виде ассоциативного массива не все поля формы, а только выборочные.

>Важно отметить, что по умолчанию к значениям применяется операция обрезания (удаляются пробелы по краям) и конвертация пустых строк в NULL. Это операцию при желании можно отключить путём правки свойства `$middleware` в файле `App\Http\Kernel.php`:

##### Получение массивов

Если данные приходят из формы не с отдельными именами , а в виде массива, то с помощью метода `input()` можно обращаться к элементам массива, используя точечную нотацию (т.е. писать индекс не в квадратных скобках, а через точку). Указанный вместо индекса элемента символ звёздочки `*` выступает в качестве квантификатора, указывая что нужно взять сразу все элементы (т.е. вернуть сам массив). Можно конечно просто указать имя массива без квантификатора, но во первых он повышает наглядность кода, а во вторых необходим в слуачях если элементы массива являются массивами и нужно сделать выборку их отдельных элементов:

```php
    Route::post('/post_array1', function (Request $request) {
        $s = 'Получен массив: ';
        foreach ( request()->input('numbs.*') as $key=>$value )
            $s .= "$key => $value | ";
        return $s . "<br>Первый элемент массива: " . $request->input('numbs.0');
    })->name('level1_post_array1');
```

### Автоматическое преобразование данных запроса в другие типы данных

Входные данные в запросах представляют из себя обычные строки. Но есть возможность прямо при получении автоматически преобразовать их в более удобные типы данных.

##### Получение булевых значений

Если значения полей формы равны строке `"true"`, `"yes"`, `"on"` или `"1"`, то с помощью Request-метода `boolean()` их можно преобразовать в булевое значение `true`, указав в аргументе имя поля. В случае другого строкового значения или его отсутвия метод вернёт `false`:

```php
    Route::post('/post_checkboxes1', function (Request $request) {
        return (($request->boolean('rb')) ? "Ответ - Да " : "Ответ - Нет ").
            '<br> Результаты вашего выбора планов: <br>' .
               (($request->boolean('cb1')) ? "А + ":"А - ") .
               (($request->boolean('cb2')) ? "Б + ":"Б - ") .
               (($request->boolean('cb3')) ? "В + ":"В - ");
    })->name('level1_post_checkboxes1');
```

##### Получение дат в виде Carbon-объектов

Для обработки даты в Laravel есть специальный класс `Carbon`, который сильно упрощает работу с датой и временем, предоставляя для этого большой набор методов. Request-метод `date()` позволяет получить из свойства формы дату в виде Carbon-объекта:

```php
	use Carbon\Carbon;  // Подключаем класс Carbon

    Route::post('/post_date1', function (Request $request) {
        $carbonDate = $request->date('event_date');
        return "Назначенная дата: " . $carbonDate->format('Y-m-d') .
               " , осталось дней: " . $carbonDate->diffInDays(Carbon::now());
    })->name('level1_post_date1');
```

Примечательной особенностью Carbon-класса является то, что он реализован через шаблон проектирования Fluent interface (текучий интерфейс) - когда методы, применяемые к объекту, возвращают либо сам этот объект изменённый объект, либо его модифицированную копию. Такой подход позволяет применять к объекту цепочки методов для его модификации, что повышает лаконичность и наглядность кода.

##### Получение строк в виде Stringable-объектов

Аналогичный подход в Laravel реализован и для текстовых строк - через специальный класс `Stringable`